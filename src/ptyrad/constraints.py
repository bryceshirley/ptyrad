...
class CombinedConstraint(torch.nn.Module):
    ...
    def __init__(self, constraint_params, device="cuda", verbose=True):
        ...
        self.device = device
        self.constraint_params = normalize_constraint_params(
            constraint_params
        )  # constraint_params should be normalized during param loading. This is just a safe-guard
    ...
    def apply_ortho_pmode(self, model, niter):
        ...
        if self._should_apply_at_iter("ortho_pmode", niter):
            model.opt_probe.data = torch.view_as_real(
                orthogonalize_modes_vec(model.get_complex_probe_view(), sort=True).contiguous()
            )  # Note that model stores the complex probe as a (pmode, Ny, Nx, 2) float tensor (real view) so we need to do some real-complex view conversion.
            probe_int = model.get_complex_probe_view().abs().pow(2)
            probe_pow = (probe_int.sum((1, 2)) / probe_int.sum()).detach().cpu().numpy().round(3)
            vprint(
                f"Apply ortho pmode constraint at iter {niter}, relative pmode power = {probe_pow}, probe int sum = {probe_int.sum():.4f}",
    ...
    def apply_probe_mask_k(self, model, niter):
        ...
        if self._should_apply_at_iter("probe_mask_k", niter):
            ...
            pmode_index = (
                (powers_cumsum > power_thresh).nonzero()[0].item()
            )  # This gives the pmode index that the cumulative power along mode dimension is greater than the power_thresh and should have mask extend to this index
            ...
            probe_k = fftshift2(
                fft2(ifftshift2(probe), norm="ortho")
            )  # probe_k at center for later masking
            probe_r = fftshift2(
                ifft2(ifftshift2(mask * probe_k), norm="ortho")
            )  # probe_r at center. Note that the norm='ortho' is explicitly specified but not needed for a round-trip
            probe_int = model.get_complex_probe_view().abs().pow(2)
            ...
            vprint(
                f"Apply Fourier-space probe amplitude constraint at iter {niter}, pmode_index = {pmode_index} when power_thresh = {power_thresh}, probe int sum = {probe_int.sum():.4f}",
                verbose=self.verbose,
            )
    ...
    def apply_fix_probe_int(self, model, niter):
        ...
        if self._should_apply_at_iter("fix_probe_int", niter):
            ...
            probe_int = model.get_complex_probe_view().abs().pow(2)
            vprint(
                f"Apply fix probe int constraint at iter {niter}, probe int sum = {probe_int.sum():.4f}",
                verbose=self.verbose,
            )
    ...
    def apply_obj_rblur(self, model, niter):
        ...
        if (
            self._should_apply_at_iter("obj_rblur", niter)
            and self.constraint_params["obj_rblur"]["std"] != 0
        ):
            ...
            if obj_type in ["amplitude", "both"]:
                model.opt_obja.data = gaussian_blur(
                    model.opt_obja, kernel_size=obj_rblur_ks, sigma=obj_rblur_std
                )
                vprint(
                    f"Apply lateral (y,x) Gaussian blur with std = {obj_rblur_std} px on obja at iter {niter}",
                    verbose=self.verbose,
                )
            if obj_type in ["phase", "both"]:
                model.opt_objp.data = gaussian_blur(
                    model.opt_objp, kernel_size=obj_rblur_ks, sigma=obj_rblur_std
                )
                vprint(
                    f"Apply lateral (y,x) Gaussian blur with std = {obj_rblur_std} px on objp at iter {niter}",
                    verbose=self.verbose,
                )
    ...
    def apply_obj_zblur(self, model, niter):
        ...
        if (
            self._should_apply_at_iter("obj_zblur", niter)
            and self.constraint_params["obj_zblur"]["std"] != 0
        ):
            ...
            if obj_type in ["amplitude", "both"]:
                ...
                model.opt_obja.data = (
                    gaussian_blur_1d(tensor, kernel_size=obj_zblur_ks, sigma=obj_zblur_std)
                    .permute(0, 3, 1, 2)
                    .contiguous()
                )  # contiguous() returns a contiguous memory layout so that DDP wouldn't complain about the stride mismatch of grad and params
                vprint(
                    f"Apply z-direction Gaussian blur with std = {obj_zblur_std} px on obja at iter {niter}",
                    verbose=self.verbose,
                )
            if obj_type in ["phase", "both"]:
                ...
                vprint(
                    f"Apply z-direction Gaussian blur with std = {obj_zblur_std} px on objp at iter {niter}",
                    verbose=self.verbose,
                )
    ...
    def apply_kr_filter(self, model, niter):
        ...
        if self._should_apply_at_iter("kr_filter", niter):
            ...
            if obj_type in ["amplitude", "both"]:
                ...
                vprint(
                    f"Apply kr_filter constraint with kr_radius = {relative_radius} on obja at iter {niter}",
                    verbose=self.verbose,
                )
            if obj_type in ["phase", "both"]:
                model.opt_objp.data = kr_filter(model.opt_objp, relative_radius, relative_width)
                vprint(
                    f"Apply kr_filter constraint with kr_radius = {relative_radius} on objp at iter {niter}",
                    verbose=self.verbose,
                )
    ...
    def apply_kz_filter(self, model, niter):
        ...
        if self._should_apply_at_iter("kz_filter", niter):
            ...
            if obj_type in ["amplitude", "both"]:
                model.opt_obja.data = kz_filter(
                    model.opt_obja, beta_regularize_layers, alpha_gaussian, obj_type="amplitude"
                )
                vprint(
                    f"Apply kz_filter constraint with beta = {beta_regularize_layers} on obja at iter {niter}",
                    verbose=self.verbose,
                )
            if obj_type in ["phase", "both"]:
                model.opt_objp.data = kz_filter(
                    model.opt_objp, beta_regularize_layers, alpha_gaussian, obj_type="phase"
                )
                vprint(
                    f"Apply kz_filter constraint with beta = {beta_regularize_layers} on objp at iter {niter}",
                    verbose=self.verbose,
                )
    ...
    def apply_complex_ratio(self, model, niter):
        ...
        if self._should_apply_at_iter("complex_ratio", niter):
            ...
            if obj_type in ["amplitude", "both"]:
                ...
                vprint(
                    f"Apply complex ratio constraint with alpha1: {alpha1}, alpha2: {alpha2}, and Cbar: {Cbar.item():.3f} on obja at iter {niter}. obja range becomes ({amin:.3f}, {amax:.3f})",
                    verbose=self.verbose,
                )
            if obj_type in ["phase", "both"]:
                ...
                vprint(
                    f"Apply complex ratio constraint with alpha1: {alpha1}, alpha2: {alpha2}, and Cbar: {Cbar.item():.3f} on objp at iter {niter}. objp range becomes ({pmin:.3f}, {pmax:.3f})",
                    verbose=self.verbose,
                )
    ...
    def apply_mirrored_amp(self, model, niter):
        ...
        if self._should_apply_at_iter("mirrored_amp", niter):
            ...
            vprint(
                f"Apply {relax_str} mirrored amplitude constraint with scale = {scale} and power = {power} on obja at iter {niter}. obja range becomes ({amin:.3f}, {amax:.3f})",
                verbose=self.verbose,
            )
    ...
    def apply_obj_z_recenter(self, model, niter):
        ...
        if self._should_apply_at_iter("obj_z_recenter", niter):
            ...
            # Update model probe
            H = near_field_evolution_torch(
                probe.shape[-2:], dx, -z_shift * dz, lambd, device=model.device
            )  # If the object is shifted along +z, then the probe should be back-propagated along -z
            model.opt_probe.data = torch.view_as_real(ifft2(H[None,] * fft2(probe)).contiguous())
            vprint(
                f"Apply obj z-recenter constraint. Complex object and probe defocus are shifted by {z_shift:.3f} slice ({(z_shift * dz):.3f} {unit_str}) along depth dimension. threshold = {threshold}, scale = {scale}, and max_shift = {max_shift}.",
                verbose=self.verbose,
            )
    ...
    def apply_obja_thresh(self, model, niter):
        ...
        if self._should_apply_at_iter("obja_thresh", niter):
            ...
            model.opt_obja.data = relax * model.opt_obja + (1 - relax) * model.opt_obja.clamp(
                min=thresh[0], max=thresh[1]
            )
            relax_str = f"relaxed ({relax}*obj + ({1 - relax}*obj_clamp))" if relax != 0 else "hard"
            vprint(
                f"Apply {relax_str} threshold constraint with thresh = {thresh} on obja at iter {niter}",
                verbose=self.verbose,
            )
    ...
    def apply_objp_postiv(self, model, niter):
        ...
        if self._should_apply_at_iter("objp_postiv", niter):
            ...
            relax_str = (
                f"relaxed ({relax}*obj + ({1 - relax}*obj_postiv))" if relax != 0 else "hard"
            )
            vprint(
                f"Apply {relax_str} positivity constraint on objp with '{mode}' mode at iter {niter}. Original min = {original_min.item():.3f}. objp range becomes ({omin:.3f}, {omax:.3f})",
                verbose=self.verbose,
            )
    ...
    def apply_pos_recenter(self, model, niter):
        ...
        if self._should_apply_at_iter("pos_recenter", niter):
            ...
            relax_str = (
                f"relaxed (pos_shifts - ({1 - relax:.3f}*original_mean))" if relax != 0 else "hard"
            )
            vprint(
                f"Apply {relax_str} position recentering constraint at iter {niter}. Original mean = {orig_mean.detach().cpu().numpy().round(3)}. probe_pos_shifts.mean(0) becomes {model.opt_probe_pos_shifts.mean(0).detach().cpu().numpy().round(3)}",
                verbose=self.verbose,
            )
    ...
    def apply_tilt_smooth(self, model, niter):
        ...
        if (
            self._should_apply_at_iter("tilt_smooth", niter)
            and self.constraint_params["tilt_smooth"]["std"] != 0
        ):
            ...
            if (
                model.opt_obj_tilts.shape[0] == 1
            ):  # obj_tilts.shape = (1,2) for tilt_type: 'all', and (N,2) for 'each'
                vprint(
                    "`tilt_smooth` constraint requires `tilt_type':'each'`, skip this constraint",
                    verbose=self.verbose,
                )
                return
            ...
            vprint(
                f"Apply Gaussian blur with std = {tilt_smooth_std} scan positions on obj_tilts at iter {niter}",
                verbose=self.verbose,
            )
...
def orthogonalize_modes_vec(modes, sort=False):
    ...
    modes_reshaped = modes.reshape(
        input_shape[0], -1
    )  # Reshape modes to have a shape of (Nmode, X*Y)
    A = torch.matmul(
        modes_reshaped, modes_reshaped.H
    )  # A = M @ M^T.conj() = M @ M^H, H is the conjugate transpose

    if (
        A.device.type == "mps"
    ):  # Temporary hack because PyTorch MPS backend doesn't seem to implement linalg.eig yet.
...
def kr_filter(obj, radius, width):
    ...
    )  # interpolate needs 2 additional dimension (N,C,...) for the input than the output dimension
    ...
    )  # Apply fft2/ifft2 for only the r(y,x) dimension so the omode and z would be broadcasted
...
def kz_filter(obj, beta_regularize_layers=1, alpha_gaussian=1, obj_type="phase"):
    ...
    W = 1 - torch.atan(
        (beta_regularize_layers * torch.abs(grid_kz) / torch.sqrt(grid_kx**2 + grid_ky**2 + 1e-3))
        ** 2
    ) / (torch.pi / 2)
    ...
    # Filter the obj with filter function Wa, take the real part because Fourier-filtered obj could contain negative values
    fobj = torch.real(
        ifftn(fftn(obj, dim=(-3, -2, -1)) * Wa[None,], dim=(-3, -2, -1))
    )  # Apply fftn/ifftn for only spatial dimension so the omode would be broadcasted

    if obj_type == "amplitude":
        fobj = (
            1 + 0.9 * (fobj - 1)
        )  # This is essentially a soft obja threshold constraint built into the kz_filter routine for obja
