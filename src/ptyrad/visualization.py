...
def plot_sigmoid_mask(Npix, relative_radius, relative_width, img=None, show_circles=False):
    ...
    fig.suptitle(
        f"Sigmoid mask with radius = {relative_radius}, width = {relative_width}", fontsize=18
    )
    ...
    if show_circles:
        circle1 = plt.Circle(
            (Npix // 2, Npix // 2),
            (relative_radius - relative_width) * Npix / 2,
            color="k",
            fill=False,
            linestyle="--",
        )
...
def plot_forward_pass(model, indices, dp_power, show_fig=True, pass_fig=False):
    ...
    with torch.no_grad():
        ...
        obja_ROI = (
            (obj_patches[..., 0] * omode_occu[:, None, None, None]).sum(1).detach().cpu().numpy()
        )  # obj_ROI = (N_i, Nz,Ny,Nx)
        objp_ROI = (
            (obj_patches[..., 1] * omode_occu[:, None, None, None]).sum(1).detach().cpu().numpy()
        )  # obj_ROI = (N_i, Nz,Ny,Nx)
...
def plot_obj_tilts_avg(avg_tilt_iters, last_n_iters=2, show_fig=True, pass_fig=False):
    ...
    iters, tilts = zip(*avg_tilt_iters, strict=False)  # Separates into two tuples
    ...
    for i, ax in enumerate(axes):
        ...
        if len(iters) > 20 and last_n_iters is not None:
            ...
            axins.plot(
                iters[-last_n_iters:],
                tilts[-last_n_iters:, i],
                marker="o",
                color=f"{'C0' if i == 0 else 'C1'}",
            )
...
def plot_obj_tilts(pos, tilts, figsize=(16, 16), show_fig=True, pass_fig=False):
    ...
    if np.allclose(tilts[:, 0], 0, atol=1e-3):
        ...
        ax.text(
            0.5,
            0.5,
            "All tilts are effectively zero (<1e-3) mrad, no quiver plot",
            ha="center",
            va="center",
            fontsize=18,
            color="gray",
...
def plot_scan_positions(
    ...
    if init_pos is None:
        plt.scatter(
            ...
            y=pos[:, 0],
            c=np.arange(len(pos)),
            s=dot_scale * np.arange(len(pos)),
            label="Scan positions",
        )
    else:
        plt.scatter(
            x=init_pos[:, 1], y=init_pos[:, 0], c="C0", s=dot_scale, label="Init scan positions"
        )
...
def plot_affine_transformation(scale, asymmetry, rotation, shear):
    ...
    plt.figure()
    plt.title(
        f"Visualize affine transformation \n (scale, asym, rot, shear) = {scale, asymmetry, rotation, shear}",
        fontsize=14,
    )
...
def plot_pos_grouping(
    pos,
    batches,
    circle_diameter=False,
    diameter_type="90%",
    figsize=(16, 8),
    dot_scale=1,
    show_fig=True,
    ...
    for i, ax in enumerate(axs):
        if i == 0:
            ...
            if circle_diameter:
                ...
                annotation_y = first_point[
                    0
                ]  # + circle_diameter / 2 + 10  # Adjust the vertical offset as needed
                ax.annotate(
                    annotation_text,
                    xy=(annotation_x - circle_diameter / 2, annotation_y - circle_diameter / 2 - 3),
                )

        ax.set_xlabel("X (obj coord, px)")
        ax.set_ylabel("Y (obj coord, px)")
        ax.set_xlim(
            pos[:, 1].min() - 10, pos[:, 1].max() + 10
        )  # Show the full range to better visualize if a sub-group (like 'center') is selected
...
def plot_slice_thickness(dz_iters, last_n_iters=10, show_fig=True, pass_fig=False):
    ...
    axs.set_title(
        f"Slice thickness (Ang): {data[-1, 1]:.5f} at iter {int(data[-1, 0])}", fontsize=16
    )
    axs.xaxis.set_major_locator(ticker.MaxNLocator(integer=True))
    plt.yticks(fontsize=14)
    plt.xticks(fontsize=14)
...
def plot_probe_modes(
    ...
    if (
        all(p is not None for p in (init_probe, opt_probe))
        and init_probe.shape[0] != opt_probe.shape[0]
    ):
        raise ValueError(
            f"All provided probes must have the same number of probe modes (axis 0), got {init_probe.shape} and {opt_probe.shape}."
        )
    ...
    # Parse variables
    non_none = [
        (label, probe, probe_pow)
        for label, probe, probe_pow in zip(labels, processed_probes, probes_pow, strict=False)
...
def plot_summary(
    ...
    indices,
    init_variables,
    selected_figs=["loss", "forward", "probe_r_amp", "probe_k_amp", "probe_k_phase", "pos"],
    collate_str="",
    show_fig=True,
    save_fig=False,
    verbose=True,
    ...
    if "loss" in selected_figs or "all" in selected_figs:
        fig_loss = plot_loss_curves(
            model.loss_iters, last_n_iters=10, show_fig=show_fig, pass_fig=True
        )
        if show_fig:
            fig_loss.show()
        if save_fig:
            fig_loss.savefig(
                safe_filename(output_path + f"/summary_loss{collate_str}{iter_str}.png")
            )
    ...
    if "forward" in selected_figs or "all" in selected_figs:
        ...
        plot_indices = indices[
            np.int32([n2 / 2 + n / 4, n2 / 2 + 3 * n / 4])
        ]  # The idea is to get 2 regions of (N/2)x(N/2) that are +-N/4 from the center of the FOV.
        ...
        if save_fig:
            fig_forward.savefig(
                safe_filename(output_path + f"/summary_forward_pass{collate_str}{iter_str}.png")
            )
    ...
    if "probe_r_amp" in selected_figs or "all" in selected_figs:
        fig_probe_modes_real_amp = plot_probe_modes(
            init_probe,
            opt_probe,
            real_or_fourier="real",
            amp_or_phase="amplitude",
        ...
        fig_probe_modes_real_amp.suptitle(
            f"Probe modes amplitude in real space at iter {niter}", fontsize=18
        )
        if show_fig:
            fig_probe_modes_real_amp.show()
        if save_fig:
            fig_probe_modes_real_amp.savefig(
                safe_filename(
                    output_path + f"/summary_probe_modes_real_amp{collate_str}{iter_str}.png"
                ),
                bbox_inches="tight",
            )

    if "probe_k_amp" in selected_figs or "all" in selected_figs:
        fig_probe_modes_fourier_amp = plot_probe_modes(
            init_probe,
            opt_probe,
            real_or_fourier="fourier",
            amp_or_phase="amplitude",
            show_fig=False,
            pass_fig=True,
        )
        fig_probe_modes_fourier_amp.suptitle(
            f"Probe modes amplitude in fourier space at iter {niter}", fontsize=18
        )
        if show_fig:
            fig_probe_modes_fourier_amp.show()
        if save_fig:
            fig_probe_modes_fourier_amp.savefig(
                safe_filename(
                    output_path + f"/summary_probe_modes_fourier_amp{collate_str}{iter_str}.png"
                ),
                bbox_inches="tight",
            )

    if "probe_k_phase" in selected_figs or "all" in selected_figs:
        fig_probe_modes_fourier_phase = plot_probe_modes(
            init_probe,
            opt_probe,
            real_or_fourier="fourier",
            amp_or_phase="phase",
        ...
        fig_probe_modes_fourier_phase.suptitle(
            f"Probe modes phase in fourier space at iter {niter}", fontsize=18
        )
        if show_fig:
            fig_probe_modes_fourier_phase.show()
        if save_fig:
            fig_probe_modes_fourier_phase.savefig(
                safe_filename(
                    output_path + f"/summary_probe_modes_fourier_phase{collate_str}{iter_str}.png"
                ),
                bbox_inches="tight",
            )
    ...
    if "tilt" in selected_figs or "all" in selected_figs:
        fig_obj_tilts, ax = plot_obj_tilts(
            pos=pos[indices], tilts=tilts[indices], show_fig=False, pass_fig=True
        )
        ax.set_title(f"Object tilts at iter {niter}", fontsize=16)
        if show_fig:
            fig_obj_tilts.show()
        if save_fig:
            fig_obj_tilts.savefig(
                safe_filename(output_path + f"/summary_obj_tilts{collate_str}{iter_str}.png")
            )

    if "tilt_avg" in selected_figs or "all" in selected_figs:
        fig_avg_obj_tilts = plot_obj_tilts_avg(
            model.avg_tilt_iters, last_n_iters=10, show_fig=show_fig, pass_fig=True
        )
        if show_fig:
            fig_avg_obj_tilts.show()
        if save_fig:
            fig_avg_obj_tilts.savefig(
                safe_filename(output_path + f"/summary_obj_tilts_avg{collate_str}{iter_str}.png")
            )

    # Slice thickness
    if "slice_thickness" in selected_figs or "all" in selected_figs:
        fig_slice_thickness = plot_slice_thickness(
            model.dz_iters, last_n_iters=10, show_fig=show_fig, pass_fig=True
        )
        if show_fig:
