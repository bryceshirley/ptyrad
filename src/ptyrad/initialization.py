...
class Initializer:
    def __init__(self, init_params, seed=None, verbose=True):
        ...
        self.init_params = deepcopy(
            init_params
        )  # This is the central params dict that will be used for initialization
        self.init_params_original = deepcopy(init_params)
        self.init_variables = {
            "random_seed": seed
        }  # This dict stores all the variables that will be used for the later ptychography reconstruction
    ...
    def init_cache(self):
        ...
        if any([self.use_cached_obj, self.use_cached_probe, self.use_cached_pos]):
            if self.cache_source == "PtyRAD":
                vprint(
                    f"Loading 'PtyRAD' file from {self.cache_path} for caching",
                    verbose=self.verbose,
                )
                self.cache_contents = load_ptyrad(self.cache_path)
            elif self.cache_source == "PtyShv":
                vprint(
                    f"Loading 'PtyShv' file from {self.cache_path} for caching",
                    verbose=self.verbose,
                )
                self.cache_contents = load_mat(
                    self.cache_path,
                    key=["object", "probe", "outputs.probe_positions"],
                    delimiter=".",
                )  # flattend dict with key using delimiter
            elif self.cache_source == "py4DSTEM":
                vprint(
                    f"Loading 'py4DSTEM' file from {self.cache_path} for caching",
                    verbose=self.verbose,
                )
                self.cache_contents = load_hdf5(self.cache_path, key=None)
            else:
                raise ValueError(
                    f"File type {source} not implemented for caching yet, please use 'PtyRAD', or 'PtyShv'!"
                )
    ...
    def init_measurements(self):
        ...
        meas_avg = meas.mean(
            0
        )  # This is equivalent to PACBED in electron microscopy. Note that if pad/resample are set to "on_the_fly", this would be different from the final one used for reconstruction.
        meas_avg_sum = (
            meas_avg.sum()
        )  # This is the total integrated intensity of the averaged diffraction pattern
        ...
        if pad_mode == "on_the_fly":
            ...
            vprint(
                f"Adjusting `meas_avg_sum` by adding {padded_int_sum:.4f} for on_the_fly meas padding",
                verbose=self.verbose,
            )
        ...
        vprint(
            f"meausrements int. statistics (min, mean, max) = ({meas.min():.4f}, {meas.mean():.4f}, {meas.max():.4f})",
            verbose=self.verbose,
        )
        vprint(
            f"measurements                      (N, Ky, Kx) = {meas.dtype}, {meas.shape}",
            verbose=self.verbose,
        )
        vprint(" ", verbose=self.verbose)
    ...
    def init_calibration(self):
        ...
        calib_mode = calib_dict[
            "mode"
        ]  # One of 'dx', 'dk', 'kMax', 'da', 'angleMax', 'RBF', 'n_alpha', or 'fitRBF'
        calib_value = calib_dict.get("value")  # fitRBF doesn't need a value here
        Npix = self.init_params_original.get(
            "meas_Npix"
        )  # Load the original Npix because init_params['meas_Npix'] could have been modified in init_measurements
        conv_angle = self.init_params.get("probe_conv_angle")
        illum_type = self.init_params.get("probe_illum_type") or "electron"
        vprint(
            f"meas_calibration mode = '{calib_mode}', value = {calib_value}", verbose=self.verbose
        )  # No need to add :.4f to value because it could be None, also it's user input so won't have too many digits

        # Load the meas_raw_avg first to ensure measurement is initialized
        try:
            meas_raw_avg = self.init_variables[
                "meas_raw_avg"
            ]  # This is the averaged measurements with only simple permuting/reshaping/flipping
        except KeyError:
            vprint(
                "Warning: 'init_variables['meas_raw_avg]' not found. Initializing measurements first for calibration...",
                verbose=self.verbose,
            )
            vprint(" ", verbose=self.verbose)
        ...
        if illum_type == "electron":
            ...
            vprint(
                "Using loaded raw averaged measurement (before crop/pad/resample) to fit RBF as a part of the meas calibration",
                verbose=self.verbose,
            )
            fitRBF = guess_radius_of_bright_field_disk(
                meas_raw_avg, thresh=calib_dict.get("thresh", 0.5)
            )

            vprint(
                f"Radius of fitted bright field disk (RBF) = {fitRBF:.2f} px with Npix = {meas_raw_avg.shape[-1]}",
                verbose=self.verbose,
            )
            vprint(
                f"Suggested probe_mask_k radius (RBF*2/Npix) > {(fitRBF * 2 / Npix):.4f}",
                verbose=self.verbose,
            )

            vprint(
                "Fitting raw averaged measurement with center, radius, and Gaussian blur std as a sanity check",
                verbose=self.verbose,
            )
            vprint(
                "Note that the fitted Gaussian blur std (detector blur) would be affected by overlapping Bragg disks",
                verbose=self.verbose,
            )
            ...
            if calib_mode == "fitRBF":
                ...
                if calib_mode != "RBF":
                    inferRBF = (
                        conv_angle / 1e3 * Npix * dx / wavelength
                    )  # We can still infer RBF using the user provided calib value
                    vprint(
                        f"Using init_params, the inferred RBF (conv_angle / 1e3 * Npix * dx / wavelength) = {inferRBF:.2f} px with Npix = {meas_raw_avg.shape[-1]}",
                        verbose=self.verbose,
                    )

            if calib_mode in ["fitRBF", "RBF"]:
                vprint(
                    "WARNING: The 'fitRBF' and 'RBF' calibration methods are highly dependent on the accuracy of user-provided experimental parameters and acquisition conditions,",
                    verbose=self.verbose,
                )
                vprint(
                    "         including convergence angle, kV, dose, specimen thickness, and collection angle for the estimation of RBF.",
                    verbose=self.verbose,
                )
                vprint(
                    "         For example, a 5-10% error in convergence angle is fairly common.",
                    verbose=self.verbose,
                )
                vprint(
                ...
                vprint(
                    "         These method should only be used as a rough estimate and not as a substitute for proper experimental calibration.",
                    verbose=self.verbose,
                )
            ...
            if calib_mode in ["RBF", "fitRBF", "n_alpha"]:
                raise ValueError(
                    f"Calibration mode '{calib_mode}' is not supported for xray. Use 'dx', 'dk', 'kMax', 'da', 'angleMax'."
                )
            # Get wavelength
            ...
            dx = infer_dx_from_params(
                **{calib_mode: calib_value, "Npix": Npix, "wavelength": wavelength}
            )

        else:
            raise ValueError(
                f"'probe_illum_type' = {illum_type} not implemented yet, please use either 'electron' or 'xray'!"
            )

        # Print the information

        vprint(
            f"dx (real space pixel size of probe and object) set to {dx:.4f} {unit_str} with Npix = {meas_raw_avg.shape[-1]}",
            verbose=self.verbose,
        )
        ...
        if crop_ranges is not None and len(crop_ranges) == 4:
            if crop_ranges[-1] is not None and len(crop_ranges[-1]) == 2:
                ...
                vprint(
                    f"Update dx to {dx:.4f} {unit_str} due to meas_crop, Npix = {Npix_modified}",
                    verbose=self.verbose,
                )
                if illum_type == "electron":
                    vprint(
                        f"Suggested probe_mask_k radius (RBF*2/Npix) changes to > {(fitRBF * 2 / Npix_modified):.4f}",
                        verbose=self.verbose,
                    )
        ...
        if pad_cfg is not None and pad_cfg.get("mode") is not None:
            ...
            vprint(
                f"Update dx to {dx:.4f} {unit_str} due to meas_pad (mode = {mode}, padding_type = {padding_type}), Npix = {target_Npix}",
                verbose=self.verbose,
            )
            if illum_type == "electron":
                vprint(
                    f"Suggested probe_mask_k radius (RBF*2/Npix) changes to > {(fitRBF * 2 / target_Npix):.4f}",
                    verbose=self.verbose,
                )
        ...
        if resample_cfg is not None and resample_cfg.get("mode") is not None:
            ...
            fitRBF_modified = (
                fitRBF * scale_factors[0]
            )  # Currently the 2 values need to be the same
            final_Npix = self.init_params["meas_Npix"]
            vprint(
                f"Update fitRBF to {fitRBF_modified:.4f} due to meas_resample (mode = {mode}, scale_factors = {scale_factors}), Npix = {final_Npix}",
                verbose=self.verbose,
            )
            if illum_type == "electron":
                vprint(
                    f"Suggested probe_mask_k radius (RBF*2/Npix) changes to > {(fitRBF_modified * 2 / final_Npix):.4f}",
                    verbose=self.verbose,
                )
    ...
    def set_variables_dict(self):
        ...
        if probe_illum_type == "electron":
            ...
            if self.verbose:
                ...
                vprint(
                    f"  RBF         = {inferRBF:.4f} px (Inferred from the given calibration, NOT necessarily from the loaded measurement data)"
                )
                vprint(f"  n_alpha     = {n_alpha:.4f} (# conv_angle)")
                vprint(
                    f"  dx          = {dx:.4f} Ang, Nyquist-limited dmin = 2*dx = {2 * dx:.4f} Ang"
                )
                vprint(
                    f"  Rayleigh-limited resolution  = {(0.61 * wavelength / conv_angle * 1e3):.4f} Ang (0.61*lambda/alpha for focused probe )"
                )
            ...
            raise ValueError(
                f"init_params['probe_illum_type'] = {probe_illum_type} not implemented yet, please use either 'electron' or 'xray'!"
            )
        ...
        self.init_variables["probe_shape"] = np.array([Npix, Npix]).astype(
            float
        )  # Keep this at float for later init_pos
    ...
    def init_probe(self):
        ...
        # Print summary
        vprint(
            f"probe                         (pmode, Ny, Nx) = {probe.dtype}, {probe.shape}",
            verbose=self.verbose,
        )
        vprint(" ", verbose=self.verbose)
    ...
    def init_pos(self):
        ...
        # Print summary
        vprint(
            f"crop_pos                                (N,2) = {crop_pos.dtype}, {crop_pos.shape}",
            verbose=self.verbose,
        )
        vprint(
            f"crop_pos 1st and last px coords (y,x)         = {crop_pos[0].tolist(), crop_pos[-1].tolist()}",
            verbose=self.verbose,
        )
        vprint(
            f"crop_pos extent (Ang)                         = {(crop_pos.max(0) - crop_pos.min(0)) * self.init_variables['dx']}",
            verbose=self.verbose,
        )
        vprint(
            f"probe_pos_shifts                        (N,2) = {probe_pos_shifts.dtype}, {probe_pos_shifts.shape}",
            verbose=self.verbose,
        )
        vprint(" ", verbose=self.verbose)
    ...
    def init_obj(self):
        ...
        vprint(
            f"object                    (omode, Nz, Ny, Nx) = {obj.dtype}, {obj.shape}",
            verbose=self.verbose,
        )
        vprint(
            f"object extent                 (Z, Y, X) (Ang) = {np.round((obj.shape[1] * dz, obj.shape[2] * dx, obj.shape[3] * dx), 4)}",
            verbose=self.verbose,
        )
        vprint(" ", verbose=self.verbose)
    ...
    def init_omode_occu(self):
        ...
        if occu_type == "custom":
            ...
            raise ValueError(
                f"Initialization method {occu_type} not implemented yet, please use 'custom' or 'uniform'!"
            )

        omode_occu = omode_occu.astype("float32")
        vprint(
            f"omode_occu                            (omode) = {omode_occu.dtype}, {omode_occu.shape}",
            verbose=self.verbose,
        )
    ...
    def init_H(self):
        ...
        vprint(
            f"Calculating H with probe_shape = {probe_shape} px, dx = {dx:.4f} {unit_str}, slice_thickness = {slice_thickness:.4f} {unit_str}, lambd = {lambd:.4f} {unit_str}",
            verbose=self.verbose,
        )
    ...
    def init_obj_tilts(self):
        ...
        if tilt_source == "custom":
            ...
            if ext in (".mat", ".h5", ".hdf5") and key is None:
                vprint(
                    f"WARNING: Couldn't find the 'key' in 'tilt_params' with file type = '{ext}'."
                )
                vprint(
                    "It is strongly recommended to provide an explicit key to better find the desired dataset, which is often much faster as well."
                )
                vprint(
                    "PtyRAD will still try to find the dataset, but you may consider setting 'key': <DATASET_KEY> inside your 'tilt_params' dict."
                )

            if ext == ".raw":
                raise ValueError(
                    "PtyRAD doesn't support loading object tilt from .raw file yet, please use other tilt_source."
                )
            obj_tilts = np.float32(load_array_from_file(**tilt_params, ndims=[2]))
            vprint(
                f"Initialized obj_tilts with loaded obj_tilts from file, mean obj_tilts = {obj_tilts.mean(0).round(2)} (theta_y, theta_x) mrad",
                verbose=self.verbose,
            )
            ...
            vprint(
                f"Initialized obj_tilts with loaded obj_tilts from PtyRAD, mean obj_tilts = {obj_tilts.mean(0).round(2)} (theta_y, theta_x) mrad",
                verbose=self.verbose,
            )
            ...
            tilt_type = (
                tilt_params.get("tilt_type") or "all"
            )  # Use the specified tilt_type when specified, fall back to 'all' for unspecified or None
            ...
            if tilt_type == "each":
                ...
                vprint(
                    f"Initialized obj_tilts with init_tilts = {init_tilts} (theta_y, theta_x) mrad",
                    verbose=self.verbose,
                )
            elif tilt_type == "all":
                ...
                raise ValueError(
                    f"Tilt type {tilt_type} not implemented yet, please use either 'each', or 'all' when initializing obj_tilts with 'simu'!"
                )

        else:
            raise ValueError(
                f"File type {tilt_source} not implemented yet, please use 'custom', 'PtyRAD', 'file', or 'simu'!"
            )

        # Print summary
        self.init_variables["obj_tilts"] = obj_tilts
        vprint(
            f"obj_tilts                              (N, 2) = {obj_tilts.dtype}, {obj_tilts.shape}",
            verbose=self.verbose,
        )
        vprint(" ", verbose=self.verbose)
    ...
    def init_check(self):
        ...
        if (
            ...
            vprint(
                f"Npix, DP measurements, probe, and H shapes are consistent as '{Npix}'",
                verbose=self.verbose,
            )
        elif (
            Npix == target_Npix == probe.shape[-2] == probe.shape[-1] == H.shape[-2] == H.shape[-1]
        ):
            vprint(
                f"Npix, DP measurements, probe, and H shapes will be consistent as '{Npix}' during on-the-fly measurement padding",
                verbose=self.verbose,
            )
        elif (
            Npix
            == floor(meas.shape[-2] * scale_factors[-2])
            == floor(meas.shape[-1] * scale_factors[-1])
            == probe.shape[-2]
            == probe.shape[-1]
            == H.shape[-2]
            == H.shape[-1]
            ...
            vprint(
                f"Npix, DP measurements, probe, and H shapes will be consistent as '{Npix}' during on-the-fly measurement resampling",
                verbose=self.verbose,
            )
        elif (
            Npix
            == floor(target_Npix * scale_factors[-2])
            == floor(target_Npix * scale_factors[-1])
            == probe.shape[-2]
            == probe.shape[-1]
            == H.shape[-2]
            == H.shape[-1]
            ...
            vprint(
                f"Npix, DP measurements, probe, and H shapes will be consistent as '{Npix}' during on-the-fly measurement padding and then resampling",
                verbose=self.verbose,
            )
        else:
            raise ValueError(
                f"Found inconsistency between Npix({Npix}), DP measurements({meas.shape[-2:]}), probe({probe.shape[-2:]}), and H({H.shape[-2:]}) shape"
            )
        ...
        if (
            N_scans
            == len(meas)
            == N_scan_slow * N_scan_fast
            == len(crop_pos)
            == len(probe_pos_shifts)
        ):
            vprint(
                f"N_scans, len(meas), N_scan_slow*N_scan_fast, len(crop_pos), and len(probe_pos_shifts) are consistent as '{N_scans}'",
                verbose=self.verbose,
            )
        else:
            raise ValueError(
                f"Found inconstency between N_scans({N_scans}), len(meas)({len(meas)}), N_scan_slow({N_scan_slow})*N_scan_fast({N_scan_fast}), len(crop_pos)({len(crop_pos)}), and len(probe_pos_shifts)({len(probe_pos_shifts)})"
            )

        # Check object shape
        if obj.shape[0] == len(omode_occu):
            vprint(
                f"obj.shape[0] is consistent with len(omode_occu) as '{obj.shape[0]}'",
                verbose=self.verbose,
            )
        else:
            raise ValueError(
                f"Found inconsistency between obj.shape[0]({obj.shape[0]}) and len(omode_occu)({len(omode_occu)})"
            )
        ...
        if obj.shape[1] == Nlayer:
            ...
            raise ValueError(
                f"Found inconsistency between obj.shape[1]({obj.shape[1]}) and Nlayer({Nlayer})"
            )

        # Check object extent and probe positions
        if (crop_pos.min(0) < 0).any():
            raise ValueError(
                f"Found invalid crop position. crop_pos.min(0) {crop_pos.min(0)} must be equal or larger than 0. Please check your position and object initialization."
            )

        if (crop_pos.max(0) + Npix - obj.shape[-2:] > 0).any():
            raise ValueError(
                f"Found invalid crop position. crop_pos.max(0) {crop_pos.max(0)} + Npix ({Npix}) = {crop_pos.max(0) + Npix} must be equal or smaller than object canvas lateral size (Ny, Nx) = {obj.shape[-2:]}. Please check your position and object initialization."
            )
        vprint(
            f"crop positions (yx_min={crop_pos.min(0)}, yx_max={crop_pos.max(0) + Npix}) are well contained inside object canvas (Ny,Nx) = {obj.shape[-2:]}.",
            verbose=self.verbose,
        )
        ...
        if len(obj_tilts) in [1, N_scans]:
            ...
            raise ValueError(
                f"Found inconsistency between len(obj_tilts) ({len(obj_tilts)}), 1, and N_scans({N_scans})"
            )
    ...
    def _set_use_cached_flags(self, source):
        ...
        if same_source_and_params(triplets[0], triplets[1]) and same_source_and_params(
    ...
    def _load_meas(self):
        ...
        if meas_source != "custom" and "path" not in meas_params:
            raise KeyError(
                f"'path' is required in 'meas_params' for source '{meas_source}'. Set 'path': <PATH_TO_YOUR_DATASET> inside your 'meas_params' dict."
            )
        ...
        if meas_source == "custom":
            if not isinstance(meas_params, np.ndarray):  # assume to be a numpy array
                raise TypeError(
                    f"'custom' source requires 'meas_params' to be a NumPy array. Got {type(meas_params)}."
                )
            meas = meas_params
            ...
            "npy",
            "raw",
        ]:  # Keep the file types for backward compatibility
            # Infer file type from extension
            ...
            if ext in (".mat", ".h5", ".hdf5") and key is None:
                vprint(
                    f"WARNING: Couldn't find the 'key' in 'meas_params' with file type = '{ext}'."
                )
                vprint(
                    "It is strongly recommended to provide an explicit key to better find the desired dataset, which is often much faster as well."
                )
                vprint(
                    "PtyRAD will still try to find the dataset, but you may consider setting 'key': <DATASET_KEY> inside your 'meas_params' dict."
                )

            # Provide default shape for .raw files if it's not specified
            if ext == ".raw" and meas_params.get("shape") is None:
                vprint(
                    f"WARNING: Couldn't find the 'shape' in 'meas_params' with file type = '{ext}'."
                )
                vprint(
                    "It is strongly recommended to provide an explicit shape to better load from .raw files"
                )
                vprint(
                    "PtyRAD will still try to load the dataset based on the provided 'init_params', but you may consider setting 'shape': (N_scans, Npix, Npix) inside your 'meas_params' dict."
                )
                meas_params["shape"] = (
            ...
            raise ValueError(
                f"Unsupported measurement source '{meas_source}'. Use 'custom' or 'file'."
            )

        meas = meas.astype("float32")
        vprint(
            "Casting measurements dtype to float32 (single precision) for computational efficiency."
        )
        vprint(
            f"Imported meausrements shape / dtype = {meas.shape}, dtype = {meas.dtype}",
            verbose=self.verbose,
        )
        vprint(
            f"Imported meausrements int. statistics (min, mean, max) = ({meas.min():.4f}, {meas.mean():.4f}, {meas.max():.4f})",
            verbose=self.verbose,
        )
        return meas
    ...
    def _process_meas(self, meas):
        ...
        self.init_variables["meas_raw_avg"] = meas.mean(
            0
        )  # Save this for initial dx calibration. The crop/pad/resample effect would be accounted accordingly in `init_calibration`
        ...
        meas = self._meas_remove_neg_values(
            meas, self.init_params.get("meas_remove_neg_values")
        )  # meas need to be positive before the padding with background fitting mode
        meas = self._meas_normalization(
            meas, self.init_params.get("meas_normalization")
        )  # The normalization is needed because the background is calculated now and it needs to match the level of the final normalized meas
    ...
    def _meas_flipT(self, meas, flipT_axes):
        ...
        try:
            ...
            raise ValueError(
                f"flipT_axes must contain values convertible to int (0 or 1). Got: {flipT_axes}"
            ) from e

        vprint(
            f"Flipping measurements with [flipud, fliplr, transpose] = {flipT_axes}",
            verbose=self.verbose,
        )
        ...
        for i, bounds in enumerate(crop_ranges):
            if bounds is None:
                ...
                try:
                    ...
                    vprint(
                        f"Cropping axis {axes_names[i]} from {start} to {stop}",
                        verbose=self.verbose,
                    )
                except Exception as e:
                    raise ValueError(
                        f"Invalid crop bounds for axis {axes_names[i]}: {bounds}"
                    ) from e

        meas = meas[slices[0], slices[1], slices[2], slices[3]]
        vprint(
            f"Cropped measurements have shape (N_slow, N_fast, ky, kx) = {meas.shape}",
            verbose=self.verbose,
        )

        # Update self.init_params
        vprint(
            "Update (Npix, N_scans, N_scan_slow, N_scan_fast) after the measurements cropping",
            verbose=self.verbose,
        )
    ...
    def _meas_remove_neg_values(self, meas, neg_cfg):
        ...
        force = neg_cfg.get(
            "force", False
        )  # Default to False so it'll skip if there's no negative values

        # Check if there are negative values
        if not (meas < 0).any():
            if not force:
                vprint(
                    "No negative values found in measurements. Skipping non-neg correction.",
                    verbose=self.verbose,
                )
                return meas
            else:
                vprint(
                    f"No negative values found in measurements, but force = '{force}' so continuing measurement negative value correction",
                    verbose=self.verbose,
                )

        vprint(
            f"Removing negative values in measurement with method = {mode} and value = {value}",
            verbose=self.verbose,
        )
        ...
        if mode == "subtract_min":
            ...
            vprint(
                f"Minimum value of {min_value:.4f} subtracted due to the positive px value constraint of measurements",
                verbose=self.verbose,
            )
            ...
            vprint(
                f"Minimum value = {meas.min():.4f}, measurements below {value} are clipped to 0 due to the positive px value constraint of measurements",
                verbose=self.verbose,
            )
            meas[meas < value] = 0
            ...
            vprint(
                f"Minimum value = {meas.min():.4f}, measurements subtracted by {value} due to the positive px value constraint of measurements",
                verbose=self.verbose,
            )
            meas -= value
            ...
            vprint(
                f"Minimum value = {meas.min():.4f}, negative values are clipped to 0 due to the positive px value constraint of measurements",
                verbose=self.verbose,
            )
            meas[meas < 0] = 0
            ...
            raise ValueError(
                f"Unsupported mode '{mode}' for handling negative values. Use 'clip_neg', 'subtract_min', 'clip_value', or 'subtract_value'."
            )

        # Final check in case the user specified value is not enough to remove all neg values
        if (meas < 0).any():
            vprint(
                f"User specified value = {value} is not enough to remove negative values, applying 0 clipping"
            )
            vprint(
                f"Minimum value of {meas.min():.4f} is clipped to 0 due to the positive px value constraint of measurements",
                verbose=self.verbose,
            )
            meas[meas < 0] = 0
    ...
    def _meas_normalization(self, meas, norm_cfg):
        ...
        vprint(
            f"Normalizing measurements with mode = '{norm_mode}' and value = '{norm_const}'",
            verbose=self.verbose,
        )
        ...
        if norm_mode == "max_at_one":
            ...
            vprint(
                f"Normalizing by max of the 2D mean pattern intensity: {normalization_const:.8g}",
                verbose=self.verbose,
            )
            ...
            vprint(
                f"Normalizing by mean of the 2D mean pattern intensity: {normalization_const:.8g}",
                verbose=self.verbose,
            )
            ...
            vprint(
                f"Normalizing by sum of the 2D mean pattern intensity: {normalization_const:.8g}",
                verbose=self.verbose,
            )
            ...
            vprint(
                f"Normalizing by user-defined constant: {normalization_const:.8g}",
                verbose=self.verbose,
            )

        else:
            raise ValueError(
                f"Unsupported normalization mode '{norm_mode}'. Use 'max_at_one', 'mean_at_one', 'sum_to_one', or 'divide_const'."
            )
    ...
    def _meas_pad(self, meas, pad_cfg):
        ...
        threshold = pad_cfg.get(
            "threshold", 70
        )  # For exp and power padding that requires fitting a thresholded mask

        vprint(
            f"Padding measurements with mode='{mode}', padding_type='{padding_type}', target_Npix={target_Npix}",
            verbose=self.verbose,
        )
        ...
        if padding_type == "constant":
            ...
            amp_padded = np.pad(
                amp_avg, ((pad_y1, pad_y2), (pad_x1, pad_x2)), mode="linear_ramp", end_values=value
            )
        elif padding_type == "exp":
            mask = create_one_hot_mask(
                amp_avg, percentile=threshold
            )  # It feels like we probably don't need to normalize meas before padding because the mask is calculated by percentile
        ...
        vprint(
            f"Original meas int sum = {meas_int_sum:.4f}, padded region int sum = {padded_int_sum:.4f}, or {padded_int_sum / meas_int_sum:.2%} more intensity after padding.",
            verbose=self.verbose,
        )
        vprint(
            "This percentage should be ideally less than 5%, or you should set a lower threshold to exclude more central region.",
            verbose=self.verbose,
        )
        ...
        if mode == "precompute":
            ...
            raise ValueError(
                f"meas_pad does not support mode = '{mode}', please choose from 'on_the_fly', 'precompute', or null"
            )

        # Update iself.init_params similar to _meas_crop
        vprint("Update Npix after the measurements padding", verbose=self.verbose)
        self.init_params["meas_Npix"] = meas_padded.shape[
            -1
        ]  # This will update Npix to target_Npix no matter what mode is used
    ...
    def _meas_resample(self, meas, resample_cfg):
        ...
        if scale_factors[0] != scale_factors[1]:
            ...
            vprint(
                f"Non-uniform scale_factors {scale_factors} detected. Using uniform scale factor: {min_scale}"
            )
        ...
        if self.init_variables.get("on_the_fly_meas_padded", None) is not None:
            mode = "on_the_fly"
            vprint(
                "'meas_resample' is set to 'on_the_fly' mode because 'meas_pad' is also set to 'on_the_fly' mode",
                verbose=self.verbose,
            )

        vprint(
            f"Resampling measurements with mode = '{mode}', scale_factors = {scale_factors}",
            verbose=self.verbose,
        )
        ...
        if mode == "precompute":
            zoom_factors = np.array(
                [1.0, *scale_factors]
            )  # scipy.ndimage.zoom applies to all axes.
            meas = zoom(
                meas, zoom_factors, order=1
            )  # bilinear (order=1) could prevent overshooting. Resampling would change the meas.sum(), but we have normalization at the end of the process.
            ...
            Npix = floor(
                Npix * scale_factors[-1]
            )  # To match the rounding logic with torch.nn.functional.interpolate()
            self.init_variables["on_the_fly_meas_scale_factors"] = scale_factors

        else:
            raise ValueError(
                f"meas_resample does not support mode = '{mode}', please choose from 'on_the_fly', 'precompute', or null"
            )
        ...
        vprint(
            f"Resampled measurements have shape (N_scans, ky, kx) = {meas.shape}",
            verbose=self.verbose,
        )
    ...
    def _meas_add_source_size(self, meas, source_size_std_ang):
        ...
        meas = meas.reshape(Nslow, Nfast, *meas.shape[-2:])
        vprint(
            f"Reshaping measurements into {meas.shape} for adding partial spatial coherence (source size) induced blurring on measurements",
            verbose=self.verbose,
        )
        ...
        vprint(
            f"Adding source size (partial spatial coherence) of Gaussian blur std = {source_size_std_px:.4f} scan_step sizes or {source_size_std_ang:.4f} Ang to measurements along the scan directions",
            verbose=self.verbose,
        )

        # Apply blur over scan dimensions (0,1)
        meas = gaussian_filter(
            meas, sigma=source_size_std_px, axes=(0, 1)
        )  # Partial spatial coherence is approximated by mixing DPs at nearby probe positions
        meas = meas.reshape(-1, meas.shape[-2], meas.shape[-1])
        vprint(f"Reshape measurements back to (N, ky, kx) = {meas.shape}", verbose=self.verbose)
    ...
    def _meas_add_detector_blur(self, meas, detector_blur_std_px):
        ...
        meas = gaussian_filter(
            meas, sigma=detector_blur_std_px, axes=(-2, -1)
        )  # Detector blur is essentially the Gaussian blur along ky, kx
        vprint(
            f"Adding detector blur (point-spread function of the detector) of Gaussian blur std = {detector_blur_std_px:.4f} px to measurements along the ky, kx directions",
            verbose=self.verbose,
        )
    ...
    def _meas_add_poisson_noise(self, meas, poisson_cfg):
        ...
        if meas.min() < 0:
            vprint(
                f"Found negative values in meas, meas.min() = {meas.min():.4g}.",
                verbose=self.verbose,
            )
            if eps > -1e-5:
                vprint(
                    f"Negative values ({meas[meas < 0].mean():.4g}) are within relative numerical tolerance (min/mean) 1e-5 , clipping negative values to 0.",
                    verbose=self.verbose,
                )
                meas[meas < 0] = 0
            else:
                raise ValueError(
                    f"meas needs to be positive before applying poisson noise, got meas.min = {meas.min():.4g}. Check your 'meas_remove_neg_values'."
                )
        ...
        if unit == "total_e_per_pattern":
            ...
            raise ValueError(
                f"Unsupported unit for Poisson noise: '{unit}'. Use 'total_e_per_pattern' or 'e_per_Ang2'."
            )

        vprint(
            f"total electron per measurement = dose x scan_step_size^2 = {dose:.3f}(e-/Ang^2) x {scan_step_size:.3f}(Ang)^2 = {total_electron:.3f}",
            verbose=self.verbose,
        )

        # Normalize meas to sum to ~ 1 before applying Poisson noise
        vprint(
            f"Before applying Poisson noise: meausrements int. statistics (min, mean, max) = ({meas.min():.4f}, {meas.mean():.5f}, {meas.max():.4f})",
            verbose=self.verbose,
        )

        normalization_const = meas.sum() / meas.shape[0]
        vprint(
            f"Normalization constant = {normalization_const:.4f}, this makes each measurement sum to ~ 1.",
            verbose=self.verbose,
        )

        meas = (
            meas / normalization_const
        )  # Make each slice of the meas to sum to ~ 1. A global normalization constant keeps the relative intensity.
        vprint(
            f"After applying normalization: meausrements int. statistics (min, mean, max) = ({meas.min():.4f}, {meas.mean():.5f}, {meas.max():.4f})",
            verbose=self.verbose,
        )
        vprint(
            f"Mean total electron per pattern = meas.sum((-2,-1)).mean(0) = ({meas.sum((-2, -1)).mean(0):.5f})",
            verbose=self.verbose,
        )
        ...
        vprint(
            f"Adding Poisson noise with a total electron per diffraction pattern of {int(total_electron)}",
            verbose=self.verbose,
        )
        vprint(
            f"After applying Poisson noise: meausrements int. statistics (min, mean, max) = ({meas.min():.4f}, {meas.mean():.5f}, {meas.max():.4f})",
            verbose=self.verbose,
        )

        meas = (
            meas * normalization_const / total_electron
        )  # Un-normalize meas back to the original scale
        vprint(
            f"After un-normalizing back to original scale: meausrements int. statistics (min, mean, max) = ({meas.min():.4f}, {meas.mean():.5f}, {meas.max():.4f})",
            verbose=self.verbose,
        )
    ...
    def _export_meas(self, export_params={}):
        ...
        if file_dir in (None, ""):
            meas_path = get_nested(
                self.init_params, key=["meas_params", "path"], safe=True, default=""
            )
        ...
        if file_dir and not os.path.exists(file_dir):
            vprint(
                f"User specified 'file_dir' = '{file_dir}' doesn't exist, creating the directory now.",
                verbose=self.verbose,
            )
    ...
    def _load_probe(self):
        ...
        if probe_source == "custom":
            ...
            raise ValueError(
                f"Unsupported probe source '{probe_source}'. Use 'custom', 'PtyRAD', 'PtyShv', 'py4DSTEM', or 'simu'."
            )
    ...
    def _load_probe_from_ptyshv(self, params: str):
        mat_path = params
        mat_version = get_matfile_version(
            mat_path
        )  # https://docs.scipy.org/doc/scipy-1.11.3/reference/generated/scipy.io.matlab.matfile_version.html
        use_h5py = mat_version[0] == 2
        probe = (
            self.cache_contents["probe"]
            if self.use_cached_probe
            else load_mat(mat_path, key="probe")
        )
        vprint(
            f"Input PtyShv probe has original shape {probe.shape}, while default PtyShv order is (Ny, Nx, pmode, vp)",
            verbose=self.verbose,
        )
        ...
        if use_h5py:
            ...
            vprint(
                f"Reverse array axes order of probe to {probe.shape} because use_h5py = {use_h5py}, which automatically reverse the order",
                verbose=self.verbose,
            )
        else:
            vprint(
                f"Keep array axes order of probe at {probe.shape} because use_h5py = {use_h5py}",
                verbose=self.verbose,
            )
        ...
        if probe.ndim == 4:
            vprint(
                "Import only the 1st variable probe mode to make a final probe with (pmode, Ny, Nx)",
                verbose=self.verbose,
            )  # I don't find variable probe modes are particularly useful for electon ptychography
            probe = probe[..., 0]
        elif probe.ndim == 2:
            vprint(
                "Expanding PtyShv probe dimension to make a final probe with (pmode, Ny, Nx)",
                verbose=self.verbose,
            )
            probe = probe[..., None]
        ...
        vprint(
            f"Permute the array axes order of probe to {probe.shape} make it (pmode, Ny, Nx)",
            verbose=self.verbose,
        )
    ...
    def _load_probe_from_py4dstem(self, params: str):
        ...
        probe = (
            self.cache_contents["probe"]
            if self.use_cached_probe
            else load_hdf5(hdf5_path, key="probe")
        )
        ...
        if probe.ndim == 2:
            vprint(
                "Expanding py4DSTEM probe dimension to make a final probe with (pmode, Ny, Nx)",
                verbose=self.verbose,
            )
            probe = probe[None, ...]
    ...
    def _simulate_probe(self, simu_params: dict, probe_illum_type: str):
        ...
        if simu_params is not None:
            vprint(
                "Using user-specified parameters in 'init_params['probe_params']' for initial probe simulation.",
                verbose=self.verbose,
            )
        else:
            vprint(
                "Using experimental parameters specified by 'init_params' for initial probe simulation.",
                verbose=self.verbose,
            )
        ...
        if probe_illum_type == "electron":
            ...
            raise ValueError(
                f"Unsupported illumination type '{probe_illum_type}'. Use 'electron' or 'xray'."
            )

        # probe is (1, Ny, Nx) after simulation, expand it to (pmode, Ny, Nx) if needed
        if simu_params["pmodes"] > 1:
            probe = make_mixed_probe(
                probe[0],
                simu_params["pmodes"],
                simu_params["pmode_init_pows"],
                verbose=self.verbose,
            )
    ...
    def _process_probe(self, probe):
        ...
        probe = self._probe_set_pmode_max(
            probe, pmode_max, pmode_init_pows, orthogonalize=True, sort=True
        )
    ...
    def _probe_set_pmode_max(
        self, probe, pmode_max, pmode_init_pows, orthogonalize=True, sort=True
    ):
        """
        ...
        pmode_init_pow = [
            min(pmode_init_pows)
        ]  # pmode_init_pows is a list of float(s), so we convert it into a list of float

        if pmode_now > pmode_max:
            vprint(
                f"pmode_now: {pmode_now} and pmode_max: {pmode_max}, capping the pmode.",
                verbose=self.verbose,
            )
            ...
            vprint(
                f"pmode_now: {pmode_now} and pmode_max: {pmode_max}, leaving the pmode unchanged.",
                verbose=self.verbose,
            )
            probe_final = probe
            ...
            vprint(
                f"pmode_now: {pmode_now} and pmode_max: {pmode_max}, padding the pmode.",
                verbose=self.verbose,
            )
            num_new_modes = pmode_max - pmode_now
            vprint(
                f"Creating {num_new_modes} new probe modes from the major mode",
                verbose=self.verbose,
            )
            mixed_probe_temp = make_mixed_probe(
                probe[0], pmode_max, pmode_init_pow, verbose=False
            )  # Take the strongest probe mode and make a temporary new mixed probe (int sum at 1)
            new_modes = (
                mixed_probe_temp[-num_new_modes:] * probe_int_sum**0.5
            )  # Normalize the new mode intensity with original intensity
            probe_final = np.concatenate(
                (probe, new_modes), axis=0
            )  # Total int = 1 + num_new_modes * pmode_init_pow, will normalize it later
    ...
    def _probe_z_shift(self, probe, prop_distance):
        ...
        if prop_distance is None or prop_distance == 0:
            ...
            vprint(
                f"Applying additional axial propagation (z) = {prop_distance} {unit_str} to the probe. Positive value means forward propagation (i.e., increasing depth/z).",
                verbose=self.verbose,
            )
    ...
    def _probe_normalize(self, probe, norm_cfg):
        ...
        try:
            ...
            vprint(
                "WARNING: Measurement average sum ('meas_avg_sum') not found. Initializing measurements first for probe normalization...",
                verbose=self.verbose,
            )
            vprint(" ", verbose=self.verbose)
            self.init_measurements()
        ...
        vprint(
            f"sum(|probe_data|**2) = {np.sum(np.abs(probe) ** 2):.2f}, while meas.mean(0).sum() = {meas_avg_sum:.2f}",
            verbose=self.verbose,
        )
    ...
    def _load_pos_from_ptyshv(self, params: str):
        ...
        mat_contents = (
            self.cache_contents
            if self.use_cached_pos
            else load_mat(
                mat_path, key=["object", "probe", "outputs.probe_positions"], delimiter="."
            )
        )
        vprint(
            f"Input PtyShv probe positions has original shape {mat_contents['outputs.probe_positions'].shape}, while default PtyShv order is (N, 2)",
            verbose=self.verbose,
        )
        ...
        if use_h5py:
            mat_contents = {
                key: arr.transpose(range(arr.ndim)[::-1]) for key, arr in mat_contents.items()
            }
            vprint(
                f"Reverse array axes order because use_h5py = {use_h5py}, which automatically reverse the order",
                verbose=self.verbose,
            )

        probe_positions = mat_contents["outputs.probe_positions"]
        probe_shape = mat_contents["probe"].shape[
            :2
        ]  # Matlab probe is (Ny,Nx,pmode,vp) or (Ny,Nx,pmode)
        obj_shape = mat_contents["object"].shape[:2]  # Matlab object is (Ny, Nx, Nz) or (Ny,Nx)
        pos_offset = (
            np.ceil((np.array(obj_shape) / 2) - (np.array(probe_shape) / 2)) - 1
        )  # For Matlab - Python index shift
        probe_positions_yx = probe_positions[
            :, [1, 0]
        ]  # The first index after shifting is the row index (along vertical axis)
    ...
    def _load_pos_from_foldslice(self, params: str):
        ...
        pos = pos + np.ceil(
            (np.array(obj_shape) / 2) - (np.array(probe_shape) / 2)
        )  # Shift to obj coordinate
        return pos

    def _simulate_pos(self, simu_params: dict):
        if simu_params is not None:
            vprint(
                "Using user-specified parameters in 'init_params['pos_params']' for initial position simulation.",
                verbose=self.verbose,
            )
        else:
            simu_params = {}
            vprint(
                "Using experimental parameters specified by 'init_params' (dx, scan_step size, N_scan_slow, N_scan_fast) for initial position simulation.",
                verbose=self.verbose,
            )
        ...
        vprint(
            f"Simulating probe positions with dx = {dx:.4f}, scan_step_size = {scan_step_size:.4f}, N_scan_fast = {N_scan_fast}, N_scan_slow = {N_scan_slow}",
            verbose=self.verbose,
        )
        pos = (
            scan_step_size
            / dx
            * np.array([(y, x) for y in range(N_scan_slow) for x in range(N_scan_fast)])
        )  # (N,2), each row is (y,x)
    ...
    def _pos_scan_flipT(self, pos, flipT_axes):
        ...
        vprint(
            f"Flipping scan pattern with [flipup, fliplr, transpose] = {flipT_axes}",
            verbose=self.verbose,
        )
        ...
        if len(flipT_axes) > 0:
            pos = pos.reshape(
                self.init_variables["N_scan_slow"], self.init_variables["N_scan_fast"], 2
            )
    ...
    def _pos_scan_affine_transform(self, pos, scan_affine):
        if scan_affine is not None:
            ...
            vprint(
                f"Applying affine transformation to scan pattern with (scale, asymmetry, rotation, shear) = {(scale, asymmetry, rotation, shear)}",
                verbose=self.verbose,
            )
            pos = pos - pos.mean(0)
    ...
    def _pos_scan_add_random_displacement(self, pos, scan_rand_std):
        if scan_rand_std is not None:
            vprint(
                f"Applying Gaussian distributed random displacement with std = {scan_rand_std} px to scan positions",
                verbose=self.verbose,
            )
            set_random_seed(seed=self.random_seed)
    ...
    def _load_obj(self):
        ...
        if obj_source == "custom":
            ...
            raise ValueError(
                f"Unsupported object source '{obj_source}'. Use 'custom', 'PtyRAD', 'PtyShv', 'py4DSTEM', or 'simu'."
            )
    ...
    def _load_obj_from_ptyshv(self, params: str):
        ...
        use_h5py = mat_version[0] == 2
        obj = (
            self.cache_contents["object"]
            if self.use_cached_obj
            else load_mat(mat_path, key="object")
        )
        vprint(
            f"Input PtyShv object has original shape {obj.shape}, while default PtyShv order is (Ny, Nx, Nz)",
            verbose=self.verbose,
        )
        ...
        vprint(
            "Expanding and permuting PtyShv object dimension to make a final object shape with (omode, Nz, Ny, Nx)",
            verbose=self.verbose,
        )
    ...
    def _load_obj_from_py4dstem(self, params: str):
        ...
        obj = (
            self.cache_contents["object"]
            if self.use_cached_obj
            else load_hdf5(hdf5_path, key="object")
        )
    ...
    def _simulate_obj(self, simu_params):
        if simu_params is not None:
            vprint(
                "Using user-specified parameters in 'init_params['obj_params']' for initial object simulation.",
                verbose=self.verbose,
            )
            obj_shape = simu_params
            if len(obj_shape) != 4:
                raise ValueError(
                    f"Input `obj_shape` = {obj_shape}, please provide a total dimension of 4 with (omode, Nz, Ny, Nx)."
                )

        else:
            vprint(
                "Using experimental parameters specified by 'init_params' for initial object simulation.",
                verbose=self.verbose,
            )
            omode = self.init_params["obj_omode_max"]
            ...
            try:
                ...
                vprint(
                    "WARNING: 'obj_lateral_extent' not found. Initializing positions first for obj_shape estimation...",
                    verbose=self.verbose,
                )
                vprint(" ", verbose=self.verbose)
                self.init_pos()
    ...
    def _obj_z_crop(self, obj, crop_range):
        ...
        try:
            ...
            vprint(
                f"Cropping object depth from z_start: {z_start} to z_end: {z_end}",
                verbose=self.verbose,
            )
        except Exception as e:
            raise ValueError(
                f"Invalid crop range for object depth: {crop_range}, object shape is {obj.shape}"
            ) from e
    ...
    def _obj_z_pad(self, obj, pad_cfg):
        ...
        vprint(f"Current object has shape (omode, Nz, Ny, Nx) = {obj.shape}", verbose=self.verbose)
        vprint(
            f"Padding object along depth with pad_layers = {pad_layers}, pad_types = {pad_types}",
            verbose=self.verbose,
        )
        ...
        def _create_z_pad(obj, num_layers, pad_type, top_or_bottom):
            ...
            # Create new layers
                ...
                if pad_type == "vacuum":
                    ...
                    new_layers_a = np.mean(
                        obja, axis=1, keepdims=True
                    )  # The variance of amplitude along depth is usually quite small so geometric mean ~= arithmetric mean, although it might feel more natural to use geometric mean.
                    new_layers_p = np.mean(
                        objp, axis=1, keepdims=True
                    )  # Note that this could be a bit biased if phase is not positively constrained. The shape is (omode, 1, Ny, Nx).
                elif pad_type == "edge":
                    if top_or_bottom == "top":
                        ...
                        raise ValueError(
                            f"top_or_bottom expects 'top', or 'bottom', got {top_or_bottom}."
                        )
                    new_layers_a = obja[:, edge_idx]  # (omode, 1, Ny, Nx)
                    new_layers_p = objp[:, edge_idx]
                else:
                    raise ValueError(
                        f"Unsupported pad_type: {pad_type}, please use 'vacuum', 'mean', or 'edge'."
                    )

                new_layers = new_layers_a * np.exp(1j * new_layers_p)
                return np.broadcast_to(new_layers, new_shape).copy().astype(obj.dtype)

        top_layers = _create_z_pad(
            obj, num_layers=pad_layer_top, pad_type=pad_type_top, top_or_bottom="top"
        )
        bottom_layers = _create_z_pad(
            obj, num_layers=pad_layer_bottom, pad_type=pad_type_bottom, top_or_bottom="bottom"
        )
    ...
    def _obj_z_resample(self, obj, resample_cfg):
        ...
        Nz_now = obj.shape[1]
        dz_now = self.init_variables[
            "slice_thickness"
        ]  # This was set by `set_variables_dict` using values in `init_params['obj_slice_thickness]`
        length_unit = self.init_variables["length_unit"]

        # Print current status
        vprint(
            f"Current object has shape (omode, Nz, Ny, Nx) = {(obj.shape)}", verbose=self.verbose
        )
        vprint(
            f"Current object has slice thickness = {dz_now:.3f} {length_unit}", verbose=self.verbose
        )
        vprint(
            f"Current object has mean(prod(amp, axis='depth')) = {np.mean(np.prod(np.abs(obj), axis=1)):.3f}, mean(sum(phase, axis='depth')) = {np.mean(np.sum(np.angle(obj), axis=1)):.3g}",
            verbose=self.verbose,
        )
        vprint(
            f"Resampling object along depth with resampling mode = '{resample_mode}', value = {resample_value}",
            verbose=self.verbose,
        )

        # Get resampled object and infer new slice thickness
        obja_resample, objp_resample = complex_object_z_resample_torch(
            obj, dz_now, resample_mode, resample_value, output_type="amp_phase", return_np=True
        )  # Output amplitude and phase separately so we can check the phase value directly
        ...
        if objp_resample.max() > 2 * np.pi:
            vprint(
                f"Warning: Resampled object phase has a maximum value = {objp_resample.max():.3f} > 2pi, this would cause phase wrapping, try using thinner slices."
            )
        ...
        # Print final status
        vprint(
            f"Resampled object has shape (omode, Nz, Ny, Nx) = {(obj_resample.shape)}",
            verbose=self.verbose,
        )
        vprint(
            f"Resampled object has slice thickness = {dz_new:.3f} {length_unit}",
            verbose=self.verbose,
        )
        vprint(
            f"Resampled object has mean(prod(amp, axis='depth')) = {np.mean(np.prod(np.abs(obj_resample), axis=1)):.3f}, mean(sum(phase, axis='depth')) = {np.mean(np.sum(np.angle(obj_resample), axis=1)):.3g}",
            verbose=self.verbose,
        )
    ...
    def _object_set_omode_max(self, obj, omode_max):
        ...
        if omode_now > omode_max:
            vprint(
                f"omode_now: {omode_now} and omode_max: {omode_max}, capping the omode.",
                verbose=self.verbose,
            )
            ...
            vprint(
                f"omode_now: {omode_now} and omode_max: {omode_max}, leaving the omode unchanged.",
                verbose=self.verbose,
            )
            obj_final = obj
            ...
            vprint(
                f"omode_now: {omode_now} and omode_max: {omode_max}, padding the omode.",
                verbose=self.verbose,
            )
            num_new_modes = omode_max - omode_now
            vprint(
                f"Creating {num_new_modes} new object modes from the mean and std of original object modes",
                verbose=self.verbose,
            )
            ...
            if (
                omode_now == 1
            ):  # There's no std when omode=1. This is rather rudimentary and we'll introduced some spatially structured noise in future release
