...
class PtychoAD(torch.nn.Module):
    ...
    Args:
        ...
        parallel (bool): Whether to use parallel in time multislice. Default is False.
        device (str): Device to run computations on. Default is 'cuda:0'.
        verbose (bool): If True, prints model summary. Default is True.
    """

    def __init__(
        self, init_variables, model_params, parallel=False, device="cuda", verbose=True
    ):
        ...
        with torch.no_grad():
            ...
            if init_variables.get("on_the_fly_meas_padded", None) is not None:
                self.meas_padded = torch.tensor(
                    init_variables["on_the_fly_meas_padded"],
                    dtype=torch.float32,
                    device=device,
                )
                self.meas_padded_idx = torch.tensor(
                    init_variables["on_the_fly_meas_padded_idx"],
                    dtype=torch.int32,
                    device=device,
                )
            else:
                self.meas_padded = None
            self.meas_scale_factors = init_variables.get(
                "on_the_fly_meas_scale_factors", None
            )
            ...
            self.opt_obja = nn.Parameter(
                torch.abs(torch.tensor(init_variables["obj"], device=device)).to(
                    torch.float32
                )
            )
            self.opt_objp = nn.Parameter(
                torch.angle(torch.tensor(init_variables["obj"], device=device)).to(
                    torch.float32
                )
            )
            self.opt_obj_tilts = nn.Parameter(
                torch.tensor(
                    init_variables["obj_tilts"], dtype=torch.float32, device=device
                )
            )
            self.opt_slice_thickness = nn.Parameter(
                torch.tensor(
                    init_variables["slice_thickness"],
                    dtype=torch.float32,
                    device=device,
                )
            )
            self.opt_probe = nn.Parameter(
                torch.view_as_real(
                    torch.tensor(
                        init_variables["probe"], dtype=torch.complex64, device=device
                    )
                )
            )  # The `torch.view_as_real` allows correct handling of DDP via NCCL even in PyTorch 2.4
            self.opt_probe_pos_shifts = nn.Parameter(
                torch.tensor(
                    init_variables["probe_pos_shifts"],
                    dtype=torch.float32,
                    device=device,
                )
            )
            ...
            self.register_buffer(
                "omode_occu",
                torch.tensor(
                    init_variables["omode_occu"], dtype=torch.float32, device=device
                ),
            )
            self.register_buffer(
                "H",
                torch.tensor(init_variables["H"], dtype=torch.complex64, device=device),
            )
            self.register_buffer(
                "measurements",
                torch.tensor(
                    init_variables["measurements"], dtype=torch.float32, device=device
                ),
            )
            self.register_buffer(
                "N_scan_slow",
                torch.tensor(
                    init_variables["N_scan_slow"], dtype=torch.int32, device=device
                ),
            )  # Saving this for reference, the cropping is based on self.obj_ROI_grid.
            self.register_buffer(
                "N_scan_fast",
                torch.tensor(
                    init_variables["N_scan_fast"], dtype=torch.int32, device=device
                ),
            )  # Saving this for reference, the cropping is based on self.obj_ROI_grid.
            self.register_buffer(
                "crop_pos",
                torch.tensor(
                    init_variables["crop_pos"], dtype=torch.int32, device=device
                ),
            )  # Saving this for reference, the cropping is based on self.obj_ROI_grid.
            self.register_buffer(
                "slice_thickness",
            ...
            self.register_buffer(
                "dx",
                torch.tensor(init_variables["dx"], dtype=torch.float32, device=device),
            )  # Saving this for reference
            self.register_buffer(
                "dk",
                torch.tensor(init_variables["dk"], dtype=torch.float32, device=device),
            )  # Saving this for reference
            self.register_buffer(
                "lambd",
                torch.tensor(
                    init_variables["lambd"], dtype=torch.float32, device=device
                ),
            )

            self.random_seed = init_variables[
                "random_seed"
            ]  # Saving this for reference
            self.length_unit = init_variables[
                "length_unit"
            ]  # Saving this for reference
            self.scan_affine = init_variables[
                "scan_affine"
            ]  # Saving this for reference
            self.tilt_obj = bool(
                self.lr_params["obj_tilts"] != 0 or torch.any(self.opt_obj_tilts)
            )  # Set tilt_obj to True if lr_params['obj_tilts'] is not 0 or we have any none-zero tilt values
            self.shift_probes = bool(
                self.lr_params["probe_pos_shifts"] != 0
            )  # Set shift_probes to True if lr_params['probe_pos_shifts'] is not 0
            self.change_thickness = bool(self.lr_params["slice_thickness"] != 0)
            self.probe_int_sum = (
                self.get_complex_probe_view().abs().pow(2).sum()
            )  # This is only used for the `fix_probe_int`
            self.loss_iters = []
            self.iter_times = []
            self.dz_iters = []
            self.avg_tilt_iters = []
    ...
    def create_grids(self):
        ...
        Noy, Nox = self.opt_objp.shape[
            -2:
        ]  # Number of object pixels in y and x directions
        ...
        ky = torch.fft.ifftshift(
            2 * torch.pi * ygrid / self.dx
        )  # Use ifftshift to shift the 0 frequency to the corner
        kx = torch.fft.ifftshift(2 * torch.pi * xgrid / self.dx)
        Ky, Kx = torch.meshgrid(ky, kx, indexing="ij")
        self.propagator_grid = torch.stack(
            [Ky, Kx], dim=0
        )  # (2,Ky,Kx), k-space grid for Fresnel propagator with 2pi absorbed
        ...
        rpy, rpx = torch.meshgrid(
            ...
            indexing="ij",
        )  # real space grid for probe in y and x directions
        ...
        self.shift_probes_grid = torch.stack(
            [kpy, kpx], dim=0
        )  # (2,Npy,Npx), normalized k-space grid stack for sub-px probe shifting
        self.shift_object_grid = torch.stack(
            [koy, kox], dim=0
        )  # (2,Noy,Nox), normalized k-space grid stack for sub-px object shifting (Implemented for completeness, not used in PtyRAD)
    ...
    def create_optimizable_params_dict(self, lr_params, verbose=True):
        ...
        for param_name, lr in lr_params.items():
            if param_name not in self.optimizable_tensors:
                raise ValueError(
                    f"WARNING: '{param_name}' is not a valid parameter name, check your `update_params` and choose from 'obja', 'objp', 'obj_tilts', 'slice_thickness', 'probe', and 'probe_pos_shifts'"
                )
            else:
                self.optimizable_tensors[param_name].requires_grad = (lr != 0) and (
                    self.start_iter[param_name] == 1
                )  # Set requires_grad based on learning rate and start_iter
                if lr != 0:
                    self.optimizable_params.append(
                        {"params": [self.optimizable_tensors[param_name]], "lr": lr}
                    )
        if verbose:
    ...
    def init_propagator_vars(self):
        ...
        tilts_y_full = (
            self.opt_obj_tilts[:, 0, None, None] / 1e3
        )  # mrad, tilts_y = (N,Y,X)
        ...
        self.Kz = torch.sqrt(
            self.k**2 - Kx**2 - Ky**2
        )  # Upper case K indicates it's a 2D tensor (Y,X)
    ...
    def print_model_summary(self):
        ...
        total_var = sum(
            tensor.numel()
            for _, tensor in self.optimizable_tensors.items()
        ...
        vprint(
            f"Overdetermined ratio      : {self.measurements.numel() / total_var:.2f}"
        )
        vprint(" ")

        vprint("### Model behavior ###")
        vprint(
            f"Obj preblur               : {True if self.obj_preblur_std is not None else False}"
        )
        ...
        vprint(
            f"Detector blur             : {True if self.detector_blur_std is not None else False}"
        )
        vprint(
            f"On-the-fly meas padding   : {True if self.meas_padded is not None else False}"
        )
        vprint(
    ...
    def get_obj_ROI(self, indices):
        ...
        obj_ROI_grid_y = (
            self.rpy_grid[None, :, :] + self.crop_pos[indices, None, None, 0]
        )
        obj_ROI_grid_x = (
            self.rpx_grid[None, :, :] + self.crop_pos[indices, None, None, 1]
        )

        object_roi = opt_obj[:, :, obj_ROI_grid_y, obj_ROI_grid_x, :].permute(
            2, 0, 1, 3, 4, 5
        )
        return object_roi
    ...
    def get_obj_patches(self, indices):
        ...
        if self.obj_preblur_std is None or self.obj_preblur_std == 0:
            ...
            obj = object_patches.permute(
                5, 0, 1, 2, 3, 4
            )  # Move the r/i to the front so it's (2,N,B,D,H,W)
            obj_shape = obj.shape
            ...
            object_patches = (
                ...
                .reshape(obj_shape)
                .permute(1, 2, 3, 4, 5, 0)
            )  # The torchvision Gaussian blur only acts on last 2 dimensions
            return object_patches
    ...
    def get_probes(self, indices):
        ...
        if self.shift_probes:
            probes = imshift_batch(
                probe,
                shifts=self.opt_probe_pos_shifts[indices],
    ...
    def get_propagators(self, indices):
        ...
        tilt_obj = (
            self.tilt_obj
        )  # Whether we need to apply tilt to the Fresnel propagator
        global_tilt = self.opt_obj_tilts.shape[0] == 1  # 'tilt_type' = 'all' or 'each'
        change_tilt = (
            self.lr_params["obj_tilts"] != 0
        )  # Whether tilts are optimized or not
        change_thickness = (
            self.change_thickness
        )  # Whether thickness is optimized or not
        ...
        if tilt_obj and change_thickness:
            ...
            H_opt_dz = torch_phasor(
                dz * Kz
            )  # H has zero frequency at the corner in k-space
            return H_opt_dz * torch_phasor(
            ...
            if change_tilt:
                ...
                return (
                    self.H_fixed_tilts_full
                    if global_tilt
                    else self.H_fixed_tilts_full[indices]
                )
    ...
    def get_propagated_probe(self, index):
        ...
        probe_prop = torch.zeros(
            (n_slices, *probe.shape), dtype=probe.dtype, device=probe.device
        )
    ...
    def get_forward_meas(self, object_patches, probes, propagators):
        ...
        if self.parallel:
            # Call the optimized Born series solver using explicit keyword arguments
    ...
    def get_measurements(self, indices=None):
        ...
        if indices is not None:
            ...
            if self.meas_padded is not None:
                canvas = torch.zeros(
                    (measurements.shape[0], *meas_padded.shape[-2:]),
                    dtype=dtype,
                    device=device,
                )
                canvas += meas_padded
                canvas[..., pad_h1:pad_h2, pad_w1:pad_w2] = (
                    measurements  # Replace the center part with the original meas
                )
                measurements = canvas

            if self.meas_scale_factors is not None and any(
                factor != 1 for factor in scale_factor
            ):
                measurements = torch.nn.functional.interpolate(
                    measurements[None,], scale_factor=scale_factor, mode="bilinear"
                )[0]  # 2D interpolate requires 4D input (N, C, H, W)
                measurements = (
                    measurements / prod(scale_factor)
                )  # This ensures the intensity scale and the integrated intensity are unchanged
            ...
            if self.meas_padded is not None or self.meas_scale_factors is not None:
                vprint(
                    f"WARNING: 'on-the-fly' measurements padding/resampling detected, but they are ignored because it may cause 'CUDA out-of-memory' when 'get_measurements()' is called without any indices. The original measurement with shape = {self.measurements.shape} is returned instead."
                )
            measurements = self.measurements
    ...
    def clear_cache(self):
        """Clear temporary attributes like cached object patches and parallel forward model caches."""
