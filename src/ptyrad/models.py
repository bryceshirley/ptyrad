...
class PtychoAD(torch.nn.Module):
    ...
    def print_model_summary(self):
        ...
        total_var = sum(
            tensor.numel() for _, tensor in self.optimizable_tensors.items() if tensor.requires_grad
        )
    ...
    def get_measurements(self, indices=None):
        ...
        scale_factor = (
            tuple(self.meas_scale_factors) if self.meas_scale_factors is not None else None
        )
        ...
        if indices is not None:
            ...
            if self.meas_padded is not None:
                canvas = torch.zeros(
                    (measurements.shape[0], *meas_padded.shape[-2:]), dtype=dtype, device=device
                )
                canvas += meas_padded
            ...
            if self.meas_scale_factors is not None and any(factor != 1 for factor in scale_factor):
                ...
                measurements = measurements / prod(
                    scale_factor
