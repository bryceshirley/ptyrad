...
class SamplerParams(BaseModel):
    ...
    name: str = Field(
        default="TPESampler", description="Sampler algorithm for hyperparameter tuning"
    )
    ...
    def set_default_sampler_config(cls, values: dict) -> dict:
        ...
        if name == "TPESampler" and "configs" not in values:
            values["configs"] = {"multivariate": True, "group": True, "constant_liar": True}
        return values
    ...
    def validate_optimizer_name(cls, v: str) -> str:
        ...
        if v == "ParitalFixedSampler":
            raise NotImplementedError(
                f"Optuna sampler '{v}' is currently not implemented in PtyRAD."
            )
        return v
...
class PrunerParams(BaseModel):
    ...
    name: str = Field(
        default="HyperbandPruner", description="Pruner algorithm for early termination"
    )
    ...
    def set_default_pruner_config(cls, values: dict) -> dict:
        ...
        if name == "HyperbandPruner" and "configs" not in values:
            values["configs"] = {"min_resource": 5, "reduction_factor": 2}
        return values
    ...
    def validate_pruner_name(cls, v: str) -> str:
        ...
        if v == "WilcoxonPruner":
            raise NotImplementedError(
                f"Optuna pruner '{v}' is currently not implemented in PtyRAD."
            )
        if v == "NopPruner":
            raise ValueError(
...
class TuneParam(BaseModel):
    ...
    suggest: Literal["int", "float", "cat"] = Field(
        description="Suggestion method (e.g., 'int', 'float', 'cat')"
    )
    ...
    def validate_kwargs(cls, v: dict[str, Any], info) -> dict[str, Any]:
        ...
        if suggest == "int":
            if not all(k in v for k in ["low", "high"]):
                raise ValueError(
                    "For suggest='int', kwargs must include 'low', 'high', and either 'step' or 'log=True'"
                )
            ...
            if step is not None and step != 1 and log is True:
                raise ValueError(
                    "For suggest='int', kwargs either have (1) step=1, log=True or (2) step=<VALUE>, log=False"
                )
            ...
            if "step" in v and v["step"] is not None and not isinstance(v["step"], (int, float)):
                raise ValueError(
                    f"kwargs.step must be an int or float or None, got {type(v['step']).__name__}"
                )
            if step is not None and log is True:
                raise ValueError(
                    "For suggest='float', kwargs either have (1) step=None, log=True or (2) step=<VALUE>, log=False"
                )
...
class TuneParams(BaseModel):
    ...
    optimizer: TuneParam = Field(
        default_factory=lambda: TuneParam(
            state=False,
            suggest="cat",
            kwargs={"choices": ["Adam", "AdamW", "RMSprop", "SGD"], "optim_configs": {}},
        ),
        description="AD Optimizer",
    )
    batch_size: TuneParam = Field(
        default_factory=lambda: TuneParam(
            state=False, suggest="int", kwargs={"low": 16, "high": 512, "log": True}
        ),
    ...
    plr: TuneParam = Field(
        default_factory=lambda: TuneParam(
            state=False, suggest="cat", kwargs={"choices": [1.0e-2, 1.0e-4, 1.0e-4]}
        ),
        description="Probe learning rate",
    )
    oalr: TuneParam = Field(
        default_factory=lambda: TuneParam(
            state=False, suggest="float", kwargs={"low": 1.0e-4, "high": 1.0e-2, "log": True}
        ),
        description="Object amplitude learning rate",
    )
    oplr: TuneParam = Field(
        default_factory=lambda: TuneParam(
            state=False, suggest="float", kwargs={"low": 1.0e-4, "high": 1.0e-2, "log": True}
        ),
        description="Object phase learning rate",
    )
    slr: TuneParam = Field(
        default_factory=lambda: TuneParam(
            state=False, suggest="float", kwargs={"low": 1.0e-4, "high": 1.0e-2, "log": True}
    ...
    tlr: TuneParam = Field(
        ...
        description="Object Tilt learning rate",
    )
    dzlr: TuneParam = Field(
        default_factory=lambda: TuneParam(
            state=False, suggest="float", kwargs={"low": 1.0e-4, "high": 1.0e-2, "log": True}
        ),
        description="Slice thickness learning rate",
    ...
    dx: TuneParam = Field(
        default_factory=lambda: TuneParam(
            state=False, suggest="float", kwargs={"low": 0.1400, "high": 0.1600, "step": 0.001}
        ),
        description="Real space pixel size (Ang)",
    )
    # Probe mode and aberration
    pmode_max: TuneParam = Field(
        default_factory=lambda: TuneParam(
            state=False, suggest="int", kwargs={"low": 1, "high": 8, "step": 1}
        ),
        description="Probe modes",
    )
    conv_angle: TuneParam = Field(
        default_factory=lambda: TuneParam(
            state=False, suggest="float", kwargs={"low": 24, "high": 26, "step": 1}
        ),
        description="Convergence angle (mrad)",
    )
    defocus: TuneParam = Field(
        default_factory=lambda: TuneParam(
            state=False, suggest="float", kwargs={"low": -50, "high": 50, "step": 0.1}
        ),
        description="Defocus (Ang)",
    )
    z_shift: TuneParam = Field(
        default_factory=lambda: TuneParam(
            state=False, suggest="float", kwargs={"low": -50, "high": 50, "step": 10}
        ),
        description="Axially (z) shift the initialized probe (Ang)",
    )
    c3: TuneParam = Field(
        default_factory=lambda: TuneParam(
            state=False, suggest="float", kwargs={"low": 4000, "high": 10000, "step": 100}
        ),
        description="C3 aberration (Ang)",
    )
    c5: TuneParam = Field(
        default_factory=lambda: TuneParam(
            state=False, suggest="float", kwargs={"low": 50000, "high": 100000, "step": 5000}
        ),
        description="C5 aberration (Ang)",
    )
    # Multislice object
    Nlayer: TuneParam = Field(
        default_factory=lambda: TuneParam(
            state=False, suggest="int", kwargs={"low": 1, "high": 8, "step": 1}
        ),
        description="Number of object layers",
    )
    dz: TuneParam = Field(
        default_factory=lambda: TuneParam(
            state=False, suggest="float", kwargs={"low": 4, "high": 8, "step": 0.5}
        ),
        description="Slice thickness (Ang)",
    )
    # Scan affine
    scale: TuneParam = Field(
        default_factory=lambda: TuneParam(
            state=True, suggest="float", kwargs={"low": 0.8, "high": 1.2, "step": 0.02}
        ),
        description="Scan affine step size scale",
    )
    asymmetry: TuneParam = Field(
        default_factory=lambda: TuneParam(
            state=False, suggest="float", kwargs={"low": -0.2, "high": 0.2, "step": 0.05}
        ),
        description="Scan affine asymmetry",
    )
    rotation: TuneParam = Field(
        default_factory=lambda: TuneParam(
            state=True, suggest="float", kwargs={"low": -4, "high": 4, "step": 0.5}
        ),
        description="Scan affine rotation (degree)",
    )
    shear: TuneParam = Field(
        default_factory=lambda: TuneParam(
            state=False, suggest="float", kwargs={"low": -4, "high": 4, "step": 0.5}
        ),
        description="Scan affine shear (degree)",
    )
    # Object tilts
    tilt_y: TuneParam = Field(
        default_factory=lambda: TuneParam(
            state=False, suggest="float", kwargs={"low": -5, "high": 5, "step": 0.5}
        ),
        description="Object tilt y (mrad)",
    )
    tilt_x: TuneParam = Field(
        default_factory=lambda: TuneParam(
            state=False, suggest="float", kwargs={"low": -5, "high": 5, "step": 0.5}
        ),
        description="Object tilt x (mrad)",
    )
...
class HypertuneParams(BaseModel):
    ...
    append_params: bool = Field(
        default=True, description="Append hyperparameter names/values to output files"
    )
    """
    Default is true.
    ...
    timeout: float | None = Field(
        default=None, ge=0.0, description="Timeout for hypertune study in seconds"
    )
    """
    ...
    sampler_params: SamplerParams = Field(
        default_factory=SamplerParams, description="Sampler configuration for hypertuning"
    )
    """
    ...
    pruner_params: PrunerParams | None = Field(
        default_factory=PrunerParams, description="Pruner configuration for early termination"
    )
    """
    ...
    storage_path: str = Field(
        default="sqlite:///hypertune.sqlite3", description="Path to SQLite database for hypertune"
    )
    """
    ...
    tune_params: TuneParams = Field(
        default_factory=TuneParams, description="Hypertunable parameters"
    )
    """
