...
class OrthoPmode(BaseModel):
    ...
    start_iter: int | None = Field(
        default=1,
        ge=1,
        description="Start iteration of applying orthogonalization to mixed-state probe",
    )
    step: int | None = Field(
        default=1,
        ge=1,
        description="Interval of iterations of applying orthogonalization to mixed-state probe",
    )
    end_iter: int | None = Field(
        default=None,
        ge=1,
        description="End iteration of applying orthogonalization to mixed-state probe",
    )
...
class ProbeMaskK(BaseModel):
    ...
    start_iter: int | None = Field(
        default=None, ge=1, description="Start iteration of applying k-space probe mask"
    )
    step: int | None = Field(
        default=1, ge=1, description="Interval of iterations of applying k-space probe mask"
    )
    end_iter: int | None = Field(
        default=None, ge=1, description="End iteration of applying k-space probe mask"
    )
    radius: float = Field(
        default=0.22, ge=0.0, description="Radius of sigmoid mask in relative kMax units"
    )
    width: float = Field(default=0.05, ge=0.0, description="Width of sigmoid mask transition")
    power_thresh: float = Field(
        default=0.95, ge=0.0, le=1.0, description="Power threshold for probe modes"
    )
...
class FixProbeInt(BaseModel):
    ...
    start_iter: int | None = Field(
        default=1, ge=1, description="Start iteration of rescaling probe intensity"
    )
    step: int | None = Field(
        default=1, ge=1, description="Interval of iterations of rescaling probe intensity"
    )
    end_iter: int | None = Field(
        default=None, ge=1, description="End iteration of rescaling probe intensity"
    )
...
class ObjRblur(BaseModel):
    ...
    start_iter: int | None = Field(
        default=1, ge=1, description="Start iteration of applying lateral Gaussian blur"
    )
    step: int | None = Field(
        default=1, ge=1, description="Interval of iterations of applying lateral Gaussian blur"
    )
    end_iter: int | None = Field(
        default=None, ge=1, description="End iteration of applying lateral Gaussian blur"
    )
    obj_type: Literal["amplitude", "phase", "both"] = Field(
        default="both", description="Object type for blur"
    )
    kernel_size: int = Field(
        default=5, ge=1, description="Kernel size for Gaussian blur (odd, >6*std+1)"
    )
...
class ObjZblur(BaseModel):
    ...
    start_iter: int | None = Field(
        default=1, ge=1, description="Start iteration of applying z-direction Gaussian blur"
    )
    step: int | None = Field(
        default=1, ge=1, description="Interval of iterations of applying z-direction Gaussian blur"
    )
    end_iter: int | None = Field(
        default=None, ge=1, description="End iteration of applying z-direction Gaussian blur"
    )
    obj_type: Literal["amplitude", "phase", "both"] = Field(
        default="both", description="Object type for blur"
    )
...
class KrFilter(BaseModel):
    ...
    start_iter: int | None = Field(
        default=None, ge=1, description="Start iteration of applying lateral Fourier filter"
    )
    step: int | None = Field(
        default=1, ge=1, description="Interval of iterations of applying lateral Fourier filter"
    )
    end_iter: int | None = Field(
        default=None, ge=1, description="End iteration of applying lateral Fourier filter"
    )
    obj_type: Literal["amplitude", "phase", "both"] = Field(
        default="both", description="Object type for filter"
    )
    radius: float = Field(
        default=0.15, ge=0.0, description="Radius of sigmoid filter in relative kMax units"
    )
...
class KzFilter(BaseModel):
    ...
    start_iter: int | None = Field(
        default=None, ge=1, description="Start iteration of applying arctan kz filter"
    )
    step: int | None = Field(
        default=1, ge=1, description="Interval of iterations of applying arctan kz filter"
    )
    end_iter: int | None = Field(
        default=None, ge=1, description="End iteration of applying arctan kz filter"
    )
    obj_type: Literal["amplitude", "phase", "both"] = Field(
        default="both", description="Object type for filter"
    )
...
class ComplexRatio(BaseModel):
    ...
    start_iter: int | None = Field(
        default=None, ge=1, description="Start iteration of applying complex constraint"
    )
    step: int | None = Field(
        default=1, ge=1, description="Interval of iterations of applying complex constraint"
    )
    end_iter: int | None = Field(
        default=None, ge=1, description="End iteration of applying complex constraint"
    )
    obj_type: Literal["amplitude", "phase", "both"] = Field(
        default="both", description="Object type for constraint"
    )
...
class MirroredAmp(BaseModel):
    ...
    start_iter: int | None = Field(
        default=1, ge=1, description="Start iteration of applying mirrored amplitude constraint"
    )
    step: int | None = Field(
        default=1,
        ge=1,
        description="Interval of iterations of applying mirrored amplitude constraint",
    )
    end_iter: int | None = Field(
        default=None, ge=1, description="End iteration of applying mirrored amplitude constraint"
    )
    relax: float = Field(
        default=0.1, ge=0.0, le=1.0, description="Relaxation parameter for thresholding"
    )
    scale: float = Field(
        default=0.03, ge=0.0, description="Scale parameter for amplitude constraint"
    )
    power: float = Field(
        default=4.0, ge=0.0, description="Power parameter for amplitude constraint"
    )
...
class ObjZRecenter(BaseModel):
    ...
    start_iter: int | None = Field(
        default=None, ge=1, description="Start iteration of applying obj Z recneter constraint"
    )
    step: int | None = Field(
        default=1, ge=1, description="Interval of iterations of applying obj Z recneter constraint"
    )
    end_iter: int | None = Field(
        default=None, ge=1, description="End iteration of applying obj Z recneter constraint"
    )
    thresh: float | None = Field(
        default=95, ge=0, le=100, description="Percentile for image thresholding"
    )
    scale: float | None = Field(
        default=1, ge=0.0, le=1.0, description="Scale parameter for measured shift"
    )
    max_shift: float | None = Field(
        default=10, ge=0.0, description="Maximum value for applied shift"
    )
...
class ObjaThresh(BaseModel):
    ...
    start_iter: int | None = Field(
        default=1, ge=1, description="Start iteration of applying object amplitude thresholding"
    )
    step: int | None = Field(
        default=1,
        ge=1,
        description="Interval of iterations of applying object amplitude thresholding",
    )
    end_iter: int | None = Field(
        default=None, ge=1, description="End iteration of applying object amplitude thresholding"
    )
    relax: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Relaxation parameter for thresholding"
    )
...
class ObjpPostiv(BaseModel):
    ...
    start_iter: int | None = Field(
        default=1, ge=1, description="Start iteration of applying positivity constraint"
    )
    step: int | None = Field(
        default=1, ge=1, description="Interval of iterations of applying positivity constraint"
    )
    end_iter: int | None = Field(
        default=None, ge=1, description="End iteration of applying positivity constraint"
    )
    relax: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Relaxation parameter for positivity"
    )
...
class PosRecenter(BaseModel):
    ...
    start_iter: int | None = Field(
        default=None,
        ge=1,
        description="Start iteration of applying position recentering constraint",
    )
    step: int | None = Field(
        default=1,
        ge=1,
        description="Interval of iterations of applying position recentering constraint",
    )
    end_iter: int | None = Field(
        default=None, ge=1, description="End iteration of applying position recentering constraint"
    )
    relax: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Relaxation parameter for position recentering"
    )
...
class TiltSmooth(BaseModel):
    ...
    start_iter: int | None = Field(
        default=None, ge=1, description="Start iteration of applying tilt smoothing"
    )
    step: int | None = Field(
        default=1, ge=1, description="Interval of iterations of applying tilt smoothing"
    )
    end_iter: int | None = Field(
        default=None, ge=1, description="End iteration of applying tilt smoothing"
    )
    std: float = Field(
        default=2.0, ge=0.0, description="Standard deviation for Gaussian blur of tilts"
    )
