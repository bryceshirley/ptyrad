...
class LossSingle(BaseModel):
    ...
    dp_pow: float = Field(
        default=0.5, description="Power to raise diffraction pattern before NRMSE calculation"
    )
...
class LossPoissn(BaseModel):
    ...
    dp_pow: float = Field(
        default=1.0, description="Power to raise diffraction pattern before Poisson likelihood"
    )
    eps: float = Field(
        default=1.0e-6, gt=0.0, description="Small constant to avoid log(0) in Poisson"
    )
...
class LossSimlar(BaseModel):
    ...
    obj_type: Literal["amplitude", "phase", "both"] = Field(
        default="both", description="Object type for similarity"
    )
    scale_factor: list[float] = Field(
        default=[1.0, 1.0, 1.0], min_items=3, max_items=3, description="Scale factors for z, y, x"
    )
...
class LossParams(BaseModel):
    ...
    loss_single: LossSingle = Field(
        default_factory=LossSingle, description="NRMSE loss for single diffraction pattern"
    )
    """
    ...
    loss_poissn: LossPoissn = Field(
        default_factory=LossPoissn, description="Poisson likelihood loss"
    )
    """
    ...
    loss_sparse: LossSparse = Field(
        default_factory=LossSparse, description="L_n norm regularization for object phase"
    )
    """
    ...
    loss_simlar: LossSimlar = Field(
        default_factory=LossSimlar, description="Similarity regularization across object modes"
    )
    """
    ...
    def check_at_least_one_data_error(self):
        ...
        if not any(data_error_states):
            raise ValueError(
                "At least one of loss_single, loss_poissn, or loss_pacbed must have state: true"
            )
