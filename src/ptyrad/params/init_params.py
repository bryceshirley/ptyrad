...
class FilePathWithKey(BaseModel):
    ...
    key: str | None = Field(default=None, description="key to the dataset")
    shape: list[int] | None = Field(
        default=None, description="Shape of the dataset for loading from .raw"
    )
    offset: int | None = Field(
        default=None, description="Offset of the dataset for loading from .raw"
    )
    gap: int | None = Field(default=None, description="Gap of the dataset for loading from .raw")


class MeasCalibration(BaseModel):
    model_config = {"extra": "forbid"}

    mode: Literal["dx", "dk", "kMax", "da", "angleMax", "n_alpha", "RBF", "fitRBF"] = Field(
        default="fitRBF", description="Mode for measurements calibration"
    )
    value: float | None = Field(
        default=None,
        gt=0.0,
        description="Value for measurements calibration. Unit: Ang, Ang-1, mrad, # of alpha, px depends on modes",
    )
...
class ObjOmodeInitOccu(BaseModel):
    ...
    occu_type: Literal["uniform", "custom"] = Field(
        default="uniform", description="Mode for object mode occupancy initialization"
    )
    init_occu: list[float] | None = Field(
        description="Value for object mode occupancy initialization"
    )
...
class MeasPad(BaseModel):
    ...
    mode: Literal["on_the_fly", "precompute"] | None = Field(
        default="on_the_fly",
        description="Padding mode for measurements. Choose between 'on_the_fly' or 'precompute', or None.",
    )
    padding_type: Literal["constant", "edge", "linear_ramp", "exp", "power"] = Field(
        default="power", description="Padding type for measurements. Suggested type is 'power'."
    )
    target_Npix: int = Field(default=256, description="Target measurement number of pixels")
    value: float | None = Field(
        default=0,
        description="Value used for padding background if mode='constant' or 'linear_ramp'.",
    )
    threshold: float | None = Field(
        default=70,
        description="Threshold value used for fitting background if mode='power' or 'exp'.",
    )
...
class MeasResample(BaseModel):
    ...
    mode: Literal["on_the_fly", "precompute"] | None = Field(
        default="on_the_fly",
        description="Resampling mode for measurements. Choose between 'on_the_fly' or 'precompute', or None.",
    )
    scale_factors: list[float] = Field(
        default=[2, 2],
        min_items=2,
        max_items=2,
        description="Resampling scale factors (2,) for measurements",
    )
...
class MeasRemoveNegValues(BaseModel):
    ...
    mode: Literal["subtract_min", "subtract_value", "clip_neg", "clip_value"] = Field(
        default="clip_neg", description="Mode to remove negative values in measurements"
    )
    value: float | None = Field(
        default=None,
        description="Value used for removing negative values in measurements if mode='subtract_value' or 'clip_value'",
    )
    force: bool = Field(
        default=False,
        description="Boolean flag to force execute the operation no matter whether measurements contain negative values or not",
    )
    ...
    def check_calibration_value(cls, values: dict) -> dict:
        ...
        if mode in ["subtract_value", "clip_value"] and value is None:
            raise KeyError(
                "'value' is required in meas_remove_neg_values for mode='subtract_value' or 'clip_value'."
            )
        return values
...
class MeasNormalization(BaseModel):
    ...
    mode: Literal["max_at_one", "mean_at_one", "sum_to_one", "divide_const"] = Field(
        default="max_at_one", description="Mode to normalize measurements intensities"
    )
    value: float | None = Field(
        default=None,
        description="Value used for normalizing measurements intensities if mode='divide_const'",
    )
...
class MeasAddPoissonNoise(BaseModel):
    ...
    unit: Literal["total_e_per_pattern", "e_per_Ang2"] = Field(
        description="Unit of dose. Choose between 'total_e_per_pattern' or 'e-per_Ang2'."
    )
    value: int | float = Field(gt=0.0, description="Dose to be added to measurements")


class MeasExport(BaseModel):
    model_config = {"extra": "forbid"}

    file_dir: str | None = Field(
        default=None, description="Output directory for exported measurements"
    )
    file_name: str = Field(
        default="ptyrad_init_meas", description="Output filename for exported measurements"
    )
    file_format: Literal["hdf5", "tif", "npy", "mat"] = Field(
        default="hdf5", description="File format for exported measurements"
    )
    output_shape: list[int] | None = Field(
        default=None, description="Output shape for exported measurements"
    )
    append_shape: bool = Field(
        default=True, description="Whether to append the shape at the end of the exported file name"
    )
...
class ObjZPad(BaseModel):
    ...
    pad_layers: list[int | None] = Field(
        default=[0, 0],
        min_length=2,
        max_length=2,
        description="Number of layers to pad on object's top and bottom surfaces. List of 2 ints.",
    )
    pad_types: list[Literal["vacuum", "mean", "edge"]] = Field(
        default=["vacuum", "vacuum"],
        min_length=2,
        max_length=2,
        description="Type of layers to pad on object's top and bottom surfaces. List of 2 strings of 'vacuum', 'mean', 'edge",
    )
...
class ObjZResample(BaseModel):
    ...
    mode: Literal[
        "scale_Nlayer", "scale_slice_thickness", "target_Nlayer", "target_slice_thickness"
    ] = Field(
        description="Resampling mode for object depth. Available options are 'scale_Nlayer', 'scale_slice_thickness', 'target_Nlayer', 'target_slice_thickness'."
    )
    value: int | float = Field(
        description="Corresponding values for the specified resampling mode for object depth."
    )
...
class TiltParams(BaseModel):
    ...
    tilt_type: Literal["all", "each"] = Field(
        default="all",
        description="Type of initial titls, can be either 'all' (1,2), or 'each' (N,2)",
    )
    init_tilts: list[list[float]] = Field(
        default=[[0, 0]], description="Initial value for (N,2) object tilts"
    )
...
class InitParams(BaseModel):
    ...
    }  # This is needed to validate np.ndarray type

    random_seed: int | None = Field(
        default=None, description="Random seed for improved reproducibility"
    )

    # Experimental params
    probe_illum_type: Literal["electron", "xray"] = Field(
        default="electron", description="Probe illumination type"
    )
    """
    ...
    probe_kv: float | None = Field(
        default=None, description="Electron acceleration voltage in kV"
    )  # Required for electron
    """
    Acceleration voltage for relativistic electron wavelength calculation
    """

    probe_conv_angle: float | None = Field(
        default=None, gt=0.0, description="Semi-convergence angle in mrad"
    )  # Required for electron
    """
    Semi-convergence angle in mrad for probe-forming aperture
    """

    probe_defocus: float | None = Field(default=0.0, description="Defocus (-C1) in Angstrom")
    """
    ...
    probe_c3: float | None = Field(
        default=0.0, description="3rd-order spherical aberration (C3) in Angstrom"
    )
    """
    3rd-order spherical aberration coefficient (C3) in Ang for the simulated probe
    """

    probe_c5: float | None = Field(
        default=0.0, description="5th-order spherical aberration (C5) in Angstrom"
    )
    """
    ...
    beam_kev: float | None = Field(default=None, description="Xray beam energy in keV")

    probe_dRn: float | None = Field(
        default=None, description="Xray probe param: Width of outermost zone (in meters)"
    )

    probe_Rn: float | None = Field(
        default=None, description="Xray probe param: Radius of outermost zone (in meters)"
    )

    probe_D_H: float | None = Field(
        default=None, description="Xray probe param: Diameter of the central beamstop (in meters)"
    )

    probe_D_FZP: float | None = Field(
        default=None, description="Xray probe param: Diameter of pinhole in meters"
    )

    probe_Ls: float | None = Field(
        default=None,
        description="Xray probe param: Distance (in meters) from the focal plane to the sample",
    )
    ...
    pos_N_scan_slow: int = Field(
        ge=1, description="Number of scan positions along slow direction"
    )  # Required
    """
    Number of scan position along slow scan direction. Usually it's the vertical direction of acquisition GUI
    """

    pos_N_scan_fast: int = Field(
        ge=1, description="Number of scan positions along fast direction"
    )  # Required
    """
    ...
    meas_calibration: MeasCalibration = Field(
        default_factory=MeasCalibration, description="Calibration mode and value"
    )
    """
    ...
    # Model complexity
    probe_pmode_max: int = Field(
        ge=1, description="Maximum number of mixed probe modes"
    )  # Required
    """
    ...
    probe_pmode_init_pows: list[float] = Field(
        default=[0.02], description="Initial power weights for probe modes"
    )
    """
    ...
    obj_omode_init_occu: ObjOmodeInitOccu = Field(
        default_factory=ObjOmodeInitOccu,
        description="Occupancy type and value for mixed-object modes",
    )
    """
    ...
    obj_slice_thickness: float = Field(
        gt=0.0, description="Slice thickness in Angstrom"
    )  # Required
    """
    ...
    # Preprocessing
    meas_permute: list[int] | None = Field(
        default=None, description="Permutation for diffraction patterns"
    )
    """
    ...
    meas_reshape: list[int] | None = Field(
        default=None, min_items=3, max_items=3, description="Reshape for diffraction patterns"
    )
    """
    ...
    meas_flipT: list[int] | None = Field(
        default=None,
        min_items=3,
        max_items=3,
        description="Flip and transpose for diffraction patterns",
    )
    """
    ...
    meas_crop: list[list[int] | None] | None = Field(
        default=None, description="Crop for 4D diffraction patterns"
    )
    """
    ...
    meas_pad: MeasPad | None = Field(default=None, description="Padding configuration for CBED")
    """
    type: dict.
    ...
    meas_resample: MeasResample | None = Field(
        default=None, description="Resampling configuration for diffraction patterns"
    )
    """
    ...
    meas_add_source_size: float | None = Field(
        default=None,
        gt=0.0,
        description="Gaussian blur std for spatial partial coherence in Angstrom",
    )
    """
    ...
    meas_add_detector_blur: float | None = Field(
        default=None, gt=0.0, description="Gaussian blur std for detector in pixels"
    )
    """
    ...
    meas_remove_neg_values: MeasRemoveNegValues = Field(
        default_factory=MeasRemoveNegValues,
        description="Preprocessing for negative values in measurements",
    )
    """
    ...
    meas_normalization: MeasNormalization = Field(
        default_factory=MeasNormalization, description="Normalization method for measurements"
    )
    """
    type: null or dict.
    ...
    meas_add_poisson_noise: MeasAddPoissonNoise | None = Field(
        default=None, description="Poisson noise configuration"
    )
    """
    type: null or dict,
    ...
    meas_export: bool | MeasExport | None = Field(
        default=None, description="Export configuration for measurements"
    )
    """
    ...
    probe_permute: list[int] | None = Field(default=None, description="Permutation for probe")
    """
    ...
    probe_z_shift: float | None = Field(
        default=None, description="Axially (z) shift the initialized probe"
    )
    """
    type: null or float.
    ...
    pos_scan_flipT: list[int] | None = Field(
        default=None, description="Flip and transpose for scan patterns"
    )
    """
    type: null or list of 3 binary booleans (0 or 1) as [flipup, fliplr, transpose] just like PtychoShleves.
    ...
    pos_scan_affine: list[float] | None = Field(
        default=None, description="Affine transformation for scan patterns"
    )
    """
    ...
    pos_scan_rand_std: float | None = Field(
        default=0.15, ge=0.0, description="Random displacement std for scan positions in pixels"
    )
    """
    ...
    obj_z_crop: list[int] | None = Field(
        default=None, description="Cropping object along depth dimension"
    )
    """
    ...
    obj_z_pad: ObjZPad | None = Field(
        default=None, description="Padding object along depth dimension"
    )
    """
    ...
    obj_z_resample: ObjZResample | None = Field(
        default=None, description="Resampling object along depth dimension"
    )
    """
    ...
    meas_source: Literal["file", "custom"] = Field(
        default="file", description="Data source for measurements"
    )
    """
    ...
    meas_params: FilePathWithKey | np.ndarray = Field(
        description="Parameters for measurement loading"
    )  # Required
    """
    ...
    probe_source: Literal["simu", "PtyRAD", "PtyShv", "py4DSTEM", "custom"] = Field(
        default="simu", description="Data source for probe"
    )
    """
    Data source of the probe. Currently supporting 'simu', 'PtyRAD', 'PtyShv', 'py4DSTEM', and 'custom'
    """

    probe_params: dict[str, Any] | pathlib.Path | np.ndarray | None = Field(
        default=None, description="Parameters for probe loading/initialization"
    )
    """
    ...
    obj_source: Literal["simu", "PtyRAD", "PtyShv", "py4DSTEM", "custom"] = Field(
        default="simu", description="Data source for object"
    )
    """
    ...
    obj_params: list[int] | pathlib.Path | np.ndarray | None = Field(
        default=None, description="Parameters for object loading/initialization"
    )
    """
    ...
    tilt_source: Literal["simu", "PtyRAD", "file", "custom"] = Field(
        default="simu", description="Data source for object tilts"
    )
    """
    Data source of the object tilts. Currently supporting 'simu', 'PtyRAD', 'file', and 'custom'
    """

    tilt_params: TiltParams | FilePathWithKey | pathlib.Path | np.ndarray = Field(
        default_factory=TiltParams, description="Parameters for object tilt loading/initialization"
    )
    """
    ...
    @classmethod
    def validate_meas_crop(cls, v: list[list[int] | None] | None) -> list[list[int] | None] | None:
            ...
            for sublist in v:
                if sublist is not None:
                    if not isinstance(sublist, list) or len(sublist) != 2:
                        raise ValueError(
                            "Each sublist in meas_crop must be a list of length 2 or None."
                        )
                    if not all(isinstance(x, int) for x in sublist):
                        raise ValueError(
                            "Each element in the meas_crop sublists must be an integer."
                        )
        return v
    ...
    @classmethod
    def validate_pos_scan_flipT(cls, v: list[int] | None) -> list[int] | None:
    ...
    @classmethod
    def validate_pos_scan_affine(cls, v: list[float] | None) -> list[float] | None:
    ...
    @classmethod
    def validate_obj_z_crop(cls, v: list[int] | None) -> list[int] | None:
    ...
    @classmethod
    def validate_meas_params(
        cls, v: FilePathWithKey | np.ndarray, info
    ) -> FilePathWithKey | np.ndarray:
        ...
        if isinstance(v, np.ndarray):
            ...
            raise ValueError(
                f"{info.field_name} must be a dict, a valid file path, or a NumPy array, got {type(v).__name__}"
            )

    @field_validator("probe_params")
    @classmethod
    def validate_probe_params(
        cls, v: dict[str, Any] | pathlib.Path | np.ndarray | None, info
    ...
    @classmethod
    def validate_obj_params(
        cls, v: list[int] | pathlib.Path | np.ndarray | None, info
    ) -> list[int] | pathlib.Path | np.ndarray | None:
        if v is None:
            return None
        ...
        if isinstance(v, np.ndarray):
            ...
            raise ValueError(
                f"{info.field_name} must be either None, a List of 4 ints, a valid file path, or a NumPy array, got {type(v).__name__}"
            )
    ...
    def validate_mode_specific_fields(self):
        ...
        if self.probe_illum_type == "electron":
            ...
            for field in required_fields:
                if getattr(self, field) is None:
                    raise ValueError(f"'{field}' must be provided when probe_illum_type='xray'.")
            # Clear irrelevant fields for clarity
