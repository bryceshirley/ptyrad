...
def run(args):
    ...
    seed = resolve_seed_priority(
        args_seed=args.seed,
        params_seed=get_nested(params, "init_params.random_seed", safe=True),
        acc=accelerator,
    )
...
def main():
    ...
    parser_run.add_argument(
        "--skip_validate",
        action="store_true",
        help="Skip parameter validation and default filling. Use only if your params file is complete and consistent.",
    )
    parser_run.add_argument(
        "--gpuid",
        type=str,
    ...
    parser_run.add_argument(
        ...
        required=False,
        default=0,
        help="Unique identifier for hypertune mode with multiple GPU workers",
    )
    parser_run.add_argument(
        "--seed", type=int, required=False, help="Random seed for improved reproducibility"
    )
    ...
    parser_export = subparsers.add_parser(
        "export-meas-init", help="Export initialized measurements file to disk"
    )
    ...
    parser_export.add_argument(
        "--output",
        type=str,
        help="Optional output path / file type (.mat, .hdf5, .tif, .npy) for the exported array",
    )
    parser_export.add_argument(
        "--reshape",
        type=int,
        nargs="+",
        help="Optional new shape for the exported array, e.g. --reshape 128 128 128 128",
    )
    parser_export.add_argument(
        "--append", action="store_true", help="Optionally appending the array shape to file name"
    )
