...
def get_EM_constants(acceleration_voltage, output_type):
    ...
    # Derived values
    gamma = (
        1 + acceleration_voltage / REST_ENERGY_E
    )  # m/m0 = 1 + e*V/m0c^2, dimensionless, Lorentz factor
    wavelength = hc / np.sqrt(
        (2 * REST_ENERGY_E + acceleration_voltage) * acceleration_voltage
    )  # Angstrom, lambda = hc/sqrt((2*m0c^2 + e*V)*e*V))
    sigma = (
        2 * np.pi * gamma * REST_MASS_E * CHARGE_E * wavelength / PLANCKS**2 * 1e-20 * 1e3
    )  # interaction parameter, 2 pi*gamma*m0*e*lambda/h^2, 1/kV-Ang
    ...
    if output_type == "gamma":
        ...
        raise ValueError(
            f"output_type '{output_type}' not implemented yet, please use 'gamma', 'wavelength', or 'sigma'!"
        )
...
def complex_object_interp3d(complex_object, zoom_factors, z_axis, use_np_or_cp="np"):
    ...
    if zoom_factors == (1, 1, 1):
        print(
            f"No interpolation is needed, returning original object with shape = {complex_object.shape}."
        )
...
def complex_object_z_resample_torch(
    obj: torch.Tensor | np.ndarray,
    dz_now: float,
    resample_mode: Literal[
        "scale_Nlayer", "scale_slice_thickness", "target_Nlayer", "target_slice_thickness"
    ],
    resample_value: float | int,
    output_type: Literal["complex", "amplitude", "phase", "amp_phase"] | None = "complex",
    return_np: bool = True,
):
    ...
    if resample_mode == "scale_Nlayer":
        ...
        raise ValueError(
            f"Supported obj_z_resample modes are 'scale_Nlayer', 'scale_slice_thickness', 'target_Nlayer', and 'target_slice_thickness', got {resample_mode}."
        )

    # Check scale factor validity
    if Nz_now * Nz_scale < 1:
        raise ValueError(
            f"Detected target Nlayer = {Nz_now * Nz_scale:.3f} < 1 (single slice), please check your 'obj_z_resampling' settings."
        )
    ...
    if obj_tensor.ndim == 3:
        ...
        raise ValueError(
            f"Complex object 3D interpolation only supports 3, 4, 5D tensor, got {obj_tensor.ndim}."
        )
    ...
    obja_resample = torch.exp(
        interpolate(torch.log(obja), size=sizes, scale_factor=scale_factors, mode="area") / Nz_scale
    )
    objp_resample = (
        interpolate(objp, size=sizes, scale_factor=scale_factors, mode="area") / Nz_scale
    )
...
def get_default_probe_simu_params(init_params):
    ...
    if probe_illum_type == "electron":
        probe_simu_params = {
            ...
            "pmodes": init_params[
                "probe_pmode_max"
            ],  # These pmodes specific entries might be used in `make_mixed_probe` during initialization
            "pmode_init_pows": init_params["probe_pmode_init_pows"],
            ## Aberration coefficients
            "df": init_params[
                "probe_defocus"
            ],  # first-order aberration (defocus) in angstrom, positive defocus here refers to actual underfocus or weaker lens strength following Kirkland's notation
...
def make_stem_probe(probe_params, verbose=True):
    ...
    psi = (
        np.exp(-1j * chi)
        * np.exp(-2 * np.pi * 1j * shifts[0] * kX)
        * np.exp(-2 * np.pi * 1j * shifts[1] * kY)
    )
    probe = mask * psi  # It's now the masked wave function at the aperture plane
    probe = fftshift(
        ifft2(ifftshift(probe))
    )  # Propagate the wave function from aperture to the sample plane.
    probe = probe / np.sqrt(
        np.sum((np.abs(probe)) ** 2)
    )  # Normalize the probe so sum(abs(probe)^2) = 1
...
def make_mixed_probe(probe, pmodes, pmode_init_pows, verbose=True):
    ...
    vprint(
        f"Start making mixed-state STEM probe with {pmodes} incoherent probe modes", verbose=verbose
    )
...
def orthogonalize_modes_vec_np(modes, sort=False):
    ...
    modes = modes.astype(
        np.complex128
    )  # temporarily cast to complex128 for more precise orthogonalization
...
def near_field_evolution(Npix_shape, dx, dz, lambd):
    ...
    H = np.fft.ifftshift(
        np.exp(1j * dz * np.sqrt(k**2 - Kx**2 - Ky**2))
...
def near_field_evolution_torch(Npix_shape, dx, dz, lambd, dtype=torch.complex64, device="cuda"):
    ...
    ygrid = (
        torch.arange(-Npix_shape[0] // 2, Npix_shape[0] // 2, device=device) + 0.5
    ) / Npix_shape[0]
    xgrid = (
        torch.arange(-Npix_shape[1] // 2, Npix_shape[1] // 2, device=device) + 0.5
    ) / Npix_shape[1]
    ...
    H = ifftshift2(
        torch.exp(1j * dz * torch.sqrt(k**2 - Kx**2 - Ky**2)),
    )  # H has zero frequency at the corner in k-space
