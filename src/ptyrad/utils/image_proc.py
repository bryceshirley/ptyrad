...
def get_center_of_mass(image, corner_centered=False):
    ...
    if corner_centered:
        grid_y, grid_x = torch.meshgrid(
            fftfreq(ny, 1 / ny, device=device), fftfreq(nx, 1 / nx, device=device), indexing="ij"
        )
    else:
        grid_y, grid_x = torch.meshgrid(
...
def get_blob_size(dx, blob, output="d90", plot_profile=False, verbose=True):
    ...
    if plot_profile:
        ...
        plt.vlines(
            x=R50 * dx, ymin=0, ymax=1, color="tab:orange", linestyle=":", label="R50"
        )  # Draw vertical lines at the data coordinate, in this case would be Ang.
        ...
        plt.vlines(
            x=radius_rms * dx, ymin=0, ymax=1, color="tab:green", linestyle=":", label="Radius_RMS"
        )
        plt.xticks(
            np.arange(num_ticks)
            * np.round(
                len(radial_profile) * dx / num_ticks,
                decimals=1 - int(np.floor(np.log10(len(radial_profile) * dx))),
            )
        )
...
def fit_cbed_pattern(image: np.ndarray, initial_guess=None, verbose=False):
    ...
    vprint(
        f"Initial guess: center=({y0_guess:.2f}, {x0_guess:.2f}), radius={r_guess:.2f}, Gaussian blur std={std_guess:.2f}",
        verbose=verbose,
    )
    ...
    if not result.success or result.fun > 0.01:
        vprint(
            "First optimization attempt didn't converge well, trying different starting points",
            verbose=verbose,
        )
        ...
        for shift_y in shift_range:
            for shift_x in shift_range:
                ...
                new_result = minimize(
                    loss, new_p0, bounds=bounds, method="L-BFGS-B", options=options
                )
                ...
                if new_result.fun < best_result.fun:
                    ...
                    if verbose:
                        vprint(
                            f"Found better solution with starting point at ({new_p0[0]:.2f}, {new_p0[1]:.2f})"
                        )
    ...
    vprint(
        f"Final fit: center=({y0:.2f}, {x0:.2f}), radius={r:.2f}, Gaussian blur std={std:.2f}",
        verbose=verbose,
    )
    return {
        "center": (y0, x0),
...
def get_local_obj_tilts(pos, objp, dx, slice_thickness, slice_indices, blob_params, window_size=9):
    ...
    print(
        f"Found {len(blobs)} blobs with mean radius of {1.414 * blobs.mean(0)[-1]:.2f} px or {dx * 1.414 * blobs.mean(0)[-1]:.2f} Ang"
    )
    ...
    shift_vecs = (
        coord_b - coord_t
    )  # This is the needed tilt to correct the obj tilt so it's pointing from top to bottom
...
def gaussian_blur_1d(tensor, kernel_size=5, sigma=0.5):
    ...
    gaussian1d = torch.nn.Conv1d(
        1, 1, kernel_size, padding="same", bias=False, padding_mode="replicate"
    )
...
def create_one_hot_mask(image, percentile):
    ...
    vprint(
        f"Using percentile = {percentile:.2f}% to create an one-hot mask for measurements amplitude background fitting"
    )
    radius_px = np.sqrt(np.abs(1 - mask).sum() / np.pi)
    radius_r = radius_px / (len(mask) // 2)
    vprint(
        f"The mask has roughly {radius_px:.2f} px in radius, or {radius_r:.2f} of the distance from center to edge of the image"
    )
...
def fit_background(image, mask, fit_type="exp"):
    ...
    if fit_type == "exp":
        ...
        popt, _ = curve_fit(
            exponential_decay, masked_r, masked_image, p0=initial_guess, bounds=bounds, maxfev=10000
        )
        vprint(
            f"Fitted a = {popt[0]:.4f}, b = {popt[1]:.4f} for exponential decay: y = a*exp(-b*r)"
        )
    elif fit_type == "power":
        initial_guess = [np.max(masked_image), 1]  # [a_guess, b_guess]
        bounds = ([0, 0], [np.inf, np.inf])  # a > 0, b > 0
        popt, _ = curve_fit(
            power_law, masked_r, masked_image, p0=initial_guess, bounds=bounds, maxfev=10000
        )
...
def imshift_batch(img, shifts, grid):
    ...
    shifts = shifts[
        (...,) + (None,) * ndim
    ]  # Expand shifts to (Nb,2,1,1,...) so shifts.ndim = ndim+2. It was written as `shifts = shifts[..., *[None]*ndim]` for Python 3.11 or above with better readability
    grid = grid[
        (slice(None),) + (None,) * (ndim - 1) + (...,)
    ]  # Expand grid to (2,1,1,...,Ny,Nx) so grid.ndim = ndim+2. It was written as `grid = grid[:,*[None]*(ndim-1), ...]` for Python 3.11 or above with better readability
    shift_y, shift_x = (
        shifts[:, 0],
        shifts[:, 1],
    )  # shift_y, shift_x are (Nb,1,1,...) with ndim singletons, so the shift_y.ndim = ndim+1
    ky, kx = (
        grid[0],
        grid[1],
    )  # ky, kx are (1,1,...,Ny,Nx) with ndim-2 singletons, so the ky.ndim = ndim+1
    phase = -2 * torch.pi * (shift_x * kx + shift_y * ky)
    w = torch_phasor(
        phase
    )  # w = (Nb, 1,1,...,Ny,Nx) so w.ndim = ndim+1. The zero frequency term of w is at the corner.
