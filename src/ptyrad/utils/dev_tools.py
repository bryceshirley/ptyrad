    ...
    for i in range(tensor.shape[0]):
        for j in range(i + 1, tensor.shape[0]):
            dot_product = torch.dot(
                tensor[i].view(-1), tensor[j].view(-1).conj()
            )  # Note that torch.dot only takes 1D tensor
            if torch.allclose(
                dot_product,
                torch.tensor(0.0, dtype=dot_product.dtype, device=dot_product.device),
                atol=atol,
            ):
                print(
                    f"Modes {i} and {j} are orthogonal with abs(dot) = {dot_product.abs().detach().cpu().numpy()}"
                )
            else:
                print(
                    f"Modes {i} and {j} are not orthogonal with abs(dot) = {dot_product.abs().detach().cpu().numpy()}"
                )
...
def yaml2json(input_filepath, output_filepath):
    ...
    with open(input_filepath) as file:
        try:
            ...
            print(
                f"YAML {input_filepath} has been successfully converted and saved to JSON {output_filepath}"
            )
