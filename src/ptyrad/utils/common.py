...
def set_accelerator():
    try:
        ...
        dataloader_config = DataLoaderConfiguration(
            split_batches=True
        )  # This supress the warning when we do `Accelerator(split_batches=True)`
        kwargs_handlers = [
        ...
        if accelerator.num_processes > device_count:
            vprint(
                f"ERROR: The specified number of processes for 'accelerate' ({accelerator.num_processes}) exceeds the number of GPUs available ({device_count})."
            )
            vprint("Please verify the following:")
            vprint(
                "  1. Check the number of GPUs available on your system with `nvidia-smi` if you're using NVIDIA GPUs."
            )
            vprint(
                "  2. If using a SLURM cluster, ensure your job script requests the correct number of GPUs (e.g., `--gres=gpu:<num_gpus>`)."
            )
            vprint(
                "  3. Ensure your environment is correctly configured to detect GPUs (e.g., CUDA drivers are installed and compatible)."
            )
            raise ValueError(
                "The number of processes exceeds the available GPUs. Please adjust your configuration."
            )
        ...
        if (
            ...
            vprint(
                "Run `accelerate launch --multi_gpu --num_processes=2 --mixed_precision='no' -m ptyrad run <PTYRAD_ARGUMENTS> --gpuid 'acc'` in your terminal"
            )
    except ImportError:
...
class CustomLogger:
    def __init__(
        ...
        **kwargs,
        ...
        formatter = logging.Formatter(
            "%(asctime)s - %(message)s" if show_timestamp else "%(message)s"
        )
        ...
        vprint(
            f"log_file       = '{self.log_file}'. If log_file = None, no log file will be created."
        )
        vprint(
            f"log_dir        = '{self.log_dir}'. If log_dir = 'auto', then log will be saved to `output_path` or 'logs/'."
        )
        vprint(
            f"flush_file     = {self.flush_file}. Automatically set to True if `log_file is not None`"
        )
        vprint(
            f"prefix_time    = {self.prefix_time}. If true, preset strings ('date', 'time', 'datetime'), or a string of time format, a datetime str is prefixed to the `log_file`."
        )
        vprint(
            f"prefix_jobid   = '{self.prefix_jobid}'. If not 0, it'll be prefixed to the log file. This is used for hypertune mode with multiple GPUs."
        )
        vprint(
            f"append_to_file = {self.append_to_file}. If true, logs will be appended to the existing file. If false, the log file will be overwritten."
        )
        vprint(
            f"show_timestamp = {self.show_timestamp}. If true, the printed information will contain a timestamp."
        )
        vprint(" ")
    ...
    def flush_to_file(self, log_dir=None, append_to_file=None):
        ...
        if self.flush_file:
            ...
            )  # Always append after initial flush
            self.file_handler.setFormatter(
                logging.Formatter("%(asctime)s - %(message)s" if show_timestamp else "%(message)s")
            )
            ...
            vprint(
                f"### Log file is not flushed (created) because log_file is set to {self.log_file} ###"
            )
        vprint(" ")
...
def print_gpu_info():
    ...
    try:
        ...
        if torch.backends.cuda.is_built() and torch.cuda.is_available():
            ...
            vprint(
                f"Available CUDA GPUs: {[torch.cuda.get_device_name(d) for d in range(torch.cuda.device_count())]}"
            )
            vprint(
                f"CUDA Compute Capability: {[f'{major}.{minor}' for (major, minor) in [torch.cuda.get_device_capability(d) for d in range(torch.cuda.device_count())]]}"
            )
            vprint(
            ...
            vprint(
                "WARNING: GPU support built with PyTorch, but could not find any existing / compatible GPU device."
            )
            ...
            vprint(
                "         -> If you believe you *do* have a GPU, please check the compatibility:"
            )
            vprint("           - Are the correct NVIDIA drivers installed?")
            vprint("           - Is your CUDA runtime version compatible with PyTorch?")
            vprint(
                "           Tips: Run `nvidia-smi` in your terminal for NVIDIA driver and CUDA runtime information."
            )
            vprint(
                "           Tips: Run `conda list torch` in your terminal (with `ptyrad` environment activated) to check the installed PyTorch version."
            )
        else:
...
def print_packages_info():
    ...
    try:
        ...
        vprint(
            f"PtyRAD is located at: {module.__file__}"
        )  # For editable install this will be in package src/, while full install would make a copy at site-packages/

        if runtime_version and metadata_version and runtime_version != metadata_version:
            vprint("WARNING: Version mismatch detected!")
            vprint(
                f"  Runtime version : {runtime_version} (retrieved from current source file: ptyrad/__init__.py)"
            )
            vprint(
                f"  Metadata version: {metadata_version} (recorded during previous `pip/conda install`)"
            )
            vprint(
                "  This likely means you downloaded new codes from repo but forgot to update the installed metadata."
            )
            vprint(
                "  This does not affect the code execution because the runtime version of code is always used, but this can lead to misleading version logs."
            )
            vprint(
                "  To fix this, re-run: `pip install -e . --no-deps` at the package root directory."
            )
    ...
    try:
        ...
        if torch.cuda.is_available():
            ...
            if gpuid < num_cuda_devices:
                ...
                vprint(
                    f"Requested CUDA device cuda:{gpuid} is out of range (only {num_cuda_devices} available). "
...
def resolve_seed_priority(args_seed, params_seed, acc):
    ...
    if args_seed is not None:
        ...
        vprint(
            f"Random seed: {seed} is set automatically because multi GPU is detected but no seed is provided"
        )
    else:
        seed = None
    vprint(" ")
...
def vprint_nested_dict(d, indent=0, verbose=True, leaf_inline_threshold=6):
    ...
    for key, value in d.items():
        if isinstance(value, dict):
            ...
            if (
                is_flat_leaf and len(value) <= leaf_inline_threshold
            ):  # Determine whether to print inline or not
...
def get_nested(d, key, delimiter=".", safe=False, default=None):
    ...
    if isinstance(key, str):
        ...
        raise TypeError(
            f"'key' must be a str, or a sequence (list, tuple) of strings, got {type(key).__name__}."
        )
...
def get_time(time_format: bool | str | None = "date") -> str:
    """
...
def handle_hdf5_types(x):
    ...
    if isinstance(x, np.ndarray) and x.dtype == [("real", "<f8"), ("imag", "<f8")]:
        vprint(
            f"Detected data.shape = {x.shape} with data.dtype = {x.dtype}. Casting back to 'complex128'."
        )
...
def list_nested_keys(hobj, delimiter=".", prefix=""):
    ...
    if isinstance(hobj, (h5py.Group, h5py.File)):
        ...
        raise ValueError(
            f"Expected hobj is an HDF5 file, HDF5 group, or a dict, got {type(hobj).__name__}."
        )
    ...
    for name, params in constraint_params.items():
        ...
        normalized_params[name] = {
            ...
            **{
                ...
                if k not in ("freq", "step", "start_iter", "end_iter")
            },  # Copy other keys
        }

    if print_freq_warning:
        vprint(
            "WARNING: For constraint_params, 'freq' is depracated since PtyRAD v0.1.0b11 and is automatically converted to 'step'."
        )
